...
fn generate_session_continuity_section_from_root(root_path: &Path, tags: &[TagItem]) -> String {
    ...
    out.push_str(&format!(
        "## Session-Kontinuität (autogeneriert — Stand: {})\n",
        stand_date
    ));
    ...
    let last_commit_opt = std::process::Command::new("git")
        .args(["-C", git_root_str, "log", "--oneline", "-1"])
        .output()
        .ok()
        .filter(|o| o.status.success() && !o.stdout.is_empty())
        .map(|o| String::from_utf8_lossy(&o.stdout).trim().to_string());

    if let Some(ref lc) = last_commit_opt {
        out.push_str(&format!("- Letzter Commit: {}\n", lc));
    } else {
        out.push_str("- Letzter Commit: unbekannt\n");
    }

    // 2. Letzter Merge-Commit (git log --oneline -1 --merges)
    let last_merge_opt = std::process::Command::new("git")
        .args(["-C", git_root_str, "log", "--oneline", "-1", "--merges"])
        .output()
        .ok()
        .filter(|o| o.status.success() && !o.stdout.is_empty())
        .map(|o| String::from_utf8_lossy(&o.stdout).trim().to_string())
    ...
    if let Some(ref lm) = last_merge_opt {
        out.push_str(&format!("- Letzter Merge: {}\n", lm));
    } else {
        out.push_str("- Letzter Merge: keine\n");
    }

    // 3. Offene CRITICAL/BLOCKER-Tags
    let critical_blocker_count = tags
        .iter()
        .filter(|t| !t.is_resolved)
        .filter(|t| {
            let sev = t.severity.as_deref().unwrap_or_default();
            sev == "CRITICAL" || sev == "BLOCKER"
        })
        .count();

    if critical_blocker_count == 0 {
        out.push_str("- Offene CRITICAL/BLOCKER-Tags: keine\n");
    } else {
        out.push_str(&format!(
            "- Offene CRITICAL/BLOCKER-Tags: {}\n",
            critical_blocker_count
        ));
    }

    // 4. Aktive Claims aus .jules/claims.json
    let claims_path = root_path.join(".jules/claims.json");
    let active_claims_count = if claims_path.is_file() {
            ...
            if let Ok(val) = serde_json::from_str::<serde_json::Value>(&content) {
                if let Some(arr) = val.get("claims").and_then(|c| c.as_array()) {
                    ...
                    arr.iter()
                        .filter(|item| {
                            let active =
                                item.get("active").and_then(|a| a.as_bool()).unwrap_or(true);
                            if !active {
                                return false;
                            }
                            if let Some(exp_str) = item.get("expires_at").and_then(|e| e.as_str()) {
    ...
    if active_claims_count == 0 {
        out.push_str("- Aktive Claims: keine\n");
    } else {
        out.push_str(&format!("- Aktive Claims: {}\n", active_claims_count));
    }

    // 5. Prompter-Data-Abstand in Commits
    let prompter_data_path = root_path.join(".jules/prompter-data.json");
    ...
    let prompter_distance_str = if let Some(ref p_head) = prompter_head_opt {
    ...
    out.push_str(&format!(
        "- Prompter-Data-Abstand: {}\n",
        prompter_distance_str
    ));

    out.push_str("<!-- AUTOGENERATED:END:SESSION_CONTINUITY -->\n");
    out
}
...
#[allow(dead_code)]
fn extract_scope(title: &str) -> String {
    if let Some(start) = title.find('(') {
        if let Some(end) = title[start..].find(')') {
            return title[start + 1..start + end].trim().to_string();
        }
    }
    String::new()
}
...
fn generate_full_working_state_from_root(
    ...
    let date_from_tags = tags
        .iter()
        .filter_map(|t| {
            if t.timestamp.len() >= 10 {
                let ymd = &t.timestamp[..10];
    ...
    let mut stand_date = chrono_or_today();
    if let Some(tag_date) = date_from_tags {
        if tag_date > stand_date {
            stand_date = tag_date;
        }
    }
    ...
    out.push_str("# Contextra — Working State\n");
    out.push_str(&format!(
...
fn generate_invariants_table_section(crates: &[CrateInfo]) -> String {
    let violations = check_dag_layer_violations(crates);
    let dag_status = if violations.is_empty() {
        "✅ Erfüllt"
    } else {
        "⚠️ Verstöße vorhanden — siehe check-dag Output"
    };

    let mut out = String::new();
    out.push_str("| Invariante | Status | Befund |\n");
    out.push_str("|---|---|---|\n");
    ...
    out.push_str("| **Disk-I/O Isolation** | ✅ Erfüllt | tokio::fs für Metadaten/Lifecycle, std::fs::File ausschließlich innerhalb spawn_blocking für Block-Level Random-Access (ADR-012). |\n");
    out.push_str("| **Snapshot Isolation** | 🟢 Vollständig | Für Storage (LSM), Text (BM25) & Vektor (HNSW) vollständig implementiert. Graph-Suche operiert auf dem aktuellen In-Memory-Zustand (ADR-024). |\n");
...
fn generate_changelog(tags: &[TagItem]) -> String {
    let mut out = String::new();
    out.push_str("# Contextra — Chronologischer Tag- & Review-Bericht\n\n");
...
fn generate_dag_topology_section(crates: &[CrateInfo]) -> String {
    ...
    if has_optional {
        out.push_str(&format!("**Aktiver Workspace-Build**: {} Workspace Crates ({} Kern-Crates + 1 optionales Crate `contextra-embed`).", crates.len(), core_crates_count));
    } else {
        out.push_str(&format!(
...
pub fn run_sync_docs(check_only: bool) -> bool {
    ...
    let invariants_content = generate_invariants_table_section(&crates);
    ...
    if check_only {
        ...
        if let Err(e) = update_markdown_section(
            ...
            println!("Successfully updated docs/ARCHITECTURE.md (INVARIANTS_TABLE section).");
        }
...
pub fn run_check_dag() -> bool {
    ...
    let known_exceptions: &[(&str, &str)] = &[
        // DAG-002: contextra-checkpoint hängt von contextra-store ab (Layer 1→1, Peer-Dep, dokumentiert)
...
mod tests {
    ...
    fn test_validate_tags_no_ts_field() {
        let tags = vec![TagItem {
            file_path: "crates/contextra-test/src/lib.rs".to_string(),
            line_num: 10,
            tag_type: "AI-TAG".to_string(),
    ...
    fn test_working_state_single_marker_block() {
        ...
        assert!(ws.contains("<!-- AUTOGENERATED:START:SESSION_CONTINUITY -->"));
        assert!(ws.contains("<!-- AUTOGENERATED:END:SESSION_CONTINUITY -->"));
    }

    #[test]
    fn test_generate_session_continuity_section_format() {
        let section = generate_session_continuity_section(&[]);
        assert!(section.contains("<!-- AUTOGENERATED:START:SESSION_CONTINUITY -->"));
        assert!(section.contains("<!-- AUTOGENERATED:END:SESSION_CONTINUITY -->"));
        assert!(section.contains("## Session-Kontinuität (autogeneriert — Stand:"));
        assert!(section.contains("- Letzter Commit:"));
        assert!(section.contains("- Letzter Merge:"));
        assert!(section.contains("- Offene CRITICAL/BLOCKER-Tags:"));
        assert!(section.contains("- Aktive Claims:"));
        assert!(section.contains("- Prompter-Data-Abstand:"));
    }

    static TEST_DIR_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

    #[test]
    fn test_sync_docs_integration_in_temp_repo_fixture() {
        ...
        let run_git = |args: &[&str]| {
            let status = std::process::Command::new("git")
                ...
                .args(args)
                .status()
                .expect("Failed to execute git command in temp fixture");
...
"#,
        ...
        let crate_dir = root.join("crates/contextra-core");
...
"#,
        ...
        .unwrap();
        fs::write(crate_dir.join("src/lib.rs"), "// Core lib\n").unwrap();

        run_git(&["add", "."]);
        run_git(&["commit", "-m", "feat(contextra-core): initial setup (#100)"]);
        ...
        assert!(full_ws.contains("<!-- AUTOGENERATED:START:SESSION_CONTINUITY -->"));
        assert!(full_ws.contains("## Session-Kontinuität (autogeneriert — Stand:"));
        assert!(full_ws.contains("feat(contextra-core): initial setup (#100)"));
        assert!(full_ws.contains("## Crate-Inventar & Status"));
        assert!(full_ws.contains("## DAG-Topologie"));
    }

    #[test]
    fn test_title_similarity_and_tokenization() {
        let title1 = "refactor(contextra-index): rename partial-rebuild-pruning feature flag";
        let title2 = "refactor(contextra-index): rename partial-rebuild-pruning feature flag to partial-index-rebuild";
        ...
        assert_eq!(extract_scope(title1), "contextra-index");
    }

    #[test]
    fn test_hash_id_collision_freedom() {
        let crate_name = "contextra-store";
        let file_path = "crates/contextra-store/src/lsm.rs";
        let line_num = 42;
    ...
    fn test_check_review_coverage_fixtures() {
        ...
        let tags_0_passes = vec![TagItem {
            file_path: "crates/contextra-store/src/lsm.rs".to_string(),
            line_num: 10,
            tag_type: "ANCHOR".to_string(),
        ...
        // Fixture 3: 2 passes from DIFFERENT independent sessions (should pass for standard anchor)
        ...
        let mut tags_unsafe_file = vec![TagItem {
            file_path: "crates/contextra-index/src/distance.rs".to_string(),
            line_num: 10,
            tag_type: "ANCHOR".to_string(),
            ...
            category: None,
            severity: None,
            id: Some("AGT-INDEX-b4f29c1d".to_string()),
            session: Some("a3f29c1d".to_string()),
            status: Some("DONE".to_string()),
            description: "SIMD Task".to_string(),
            is_resolved: true,
        }];
        ...
        assert!(
            !run_check_review_coverage(&tags_unsafe_file),
        ...
        tags_unsafe_3_passes.push(TagItem {
            ...
            severity: None,
            id: Some("AGT-INDEX-b4f29c1d".to_string()),
            session: Some("d0a1b2c3".to_string()),
            status: Some("PASS".to_string()),
...
"#;
    ...
    fn test_check_consistency_fails_on_crate_missing_agents_md() {
        ...
        let crate_dir = dir.path().join("crates/contextra-fake");
        fs::create_dir_all(&crate_dir).unwrap();

        let crates = vec![CrateInfo {
            name: "contextra-fake".to_string(),
            path: "crates/contextra-fake".to_string(),
            layer: 1,
            loc: 10,
...
"#;
    ...
    fn test_context_tags_filtering_by_crate() {
        let sample_tags = vec![
            ...
            TagItem {
                file_path: "crates/contextra-index/src/hnsw.rs".to_string(),
                line_num: 99,
                tag_type: "AI-TAG".to_string(),
        ...
        let filter = parse_context_tag_args(&["--crate=contextra-store".to_string()]);
        let res = filter_tags(&sample_tags, &filter);
        assert_eq!(res.len(), 1);
        assert_eq!(res[0].krate, "contextra-store");
    ...
    fn test_check_dag_layer_violations_no_violations() {
        let crates = vec![
            CrateInfo {
                name: "contextra-core".to_string(),
                path: "crates/contextra-core".to_string(),
                layer: 0,
                loc: 100,
                status: "🟢 stable".to_string(),
            ...
            CrateInfo {
                name: "contextra-store".to_string(),
                path: "crates/contextra-store".to_string(),
                layer: 1,
                loc: 100,
                status: "🟢 stable".to_string(),
    ...
    fn test_check_dag_layer_violations_detected() {
        let crates = vec![
            ...
            CrateInfo {
                ...
                path: "crates/contextra-ollama".to_string(),
                layer: 3,
                loc: 100,
                status: "🟢 stable".to_string(),
