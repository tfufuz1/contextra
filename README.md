...
MemFuse is an embedded, multi-signal hybrid search vector database written in Rust. It combines dense vector embeddings, full-text BM25 search, graph traversals, and score calibration within a single low-latency local storage engine.

> **Development Status: Alpha**
> MemFuse is currently under active development. Core Rust storage, vector indexing, full-text search, and multi-signal fusion engines are fully implemented and tested, but APIs may evolve prior to a 1.0 release.
...
| **Python Bindings (`memfuse-py`)** | 🟡 Alpha | Basic PyO3 FFI bindings (see status in crate documentation). |
| **Model Context Protocol (`memfuse-mcp`)** | 🟡 Alpha | Local MCP server interface for AI agents. |

---

## Installation

Add `memfuse-db` to your `Cargo.toml`:
...
- **Python Bindings:** See [`crates/memfuse-py/README.md`](crates/memfuse-py/README.md) for installation and status.
- **Model Context Protocol Server:** See [`crates/memfuse-mcp/README.md`](crates/memfuse-mcp/README.md) for setup with Claude Desktop / Cursor.
...
- **Normative Architecture Specification:** [`docs/specs/MEMFUSE_SPEC_v2.md`](docs/specs/MEMFUSE_SPEC_v2.md)

---

## License

MemFuse is dual-licensed under either of the following licenses at your option:
