//! Resource budget management for Contextra.
...
impl TokenBudget {
    ...
    ///
    /// Returns `Err(ContextraError::Internal(...))` if available budget is 0 or less than `tokens`.
...
impl ResourceTracker {
    ...
    ///
    /// # Errors
    /// Returns `Err(ContextraError::MemoryBudgetExceeded)` if consuming `bytes`
...
mod tests {
    ...
    fn test_budget_exceeded() {
        ...
        match result.err().unwrap() {
            // unwrap
            // unwrap
            ContextraError::MemoryBudgetExceeded { limit_mb, .. } => {
