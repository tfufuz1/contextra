...
impl ContextChunk {
    ...
    /// Verwendet für Budget-Management in `contextra-db/src/context.rs`.
...
mod tests {
    ...
    fn test_fusion_weights_invalid_sum() {
        ...
        if let Err(ContextraError::InvalidInput(msg)) = result {
    ...
    fn test_fusion_weights_nan() {
        ...
        assert!(res.is_err());
        if let Err(ContextraError::InvalidInput(msg)) = res {
    ...
    proptest::proptest! {
        ...
        fn prop_fusion_weights_never_panics(
            ...
            if v.is_nan() || t.is_nan() || g.is_nan() {
                ...
                if let ContextraError::InvalidInput(msg) = err {
