...
async fn test_guard_uncommitted_drop_with_newer_committed_tx_preserves_newer_tx() {
    ...
    if let Err(ContextraError::Transaction(msg)) = res {
        assert!(msg.contains("Serialization barrier violation"));
    } else {
        panic!("Expected ContextraError::Transaction serialization barrier error");
    }
}
...
async fn test_drop_no_syscall_on_worker() {
    ...
    let registry = Arc::new(contextra_checkpoint::InstanceOrphanRegistry::new(
        &orphan_file,
    ));
    let storage = Arc::new(TrackingMockStorage::new());
    ...
    // 1. Drop PinGuard
    {
        let pin_guard = contextra_checkpoint::PinGuard::pin(storage.clone(), 123, registry.clone())
            .await
            .unwrap();
        drop(pin_guard);
    }
...
async fn test_pin_guard_drop_latency_sub_millisecond() {
    ...
    let pin_guard = contextra_checkpoint::PinGuard::pin(storage, 12345, registry.clone())
        .await
        .unwrap();
