...
async fn proof_source_doc_ids_set_after_compact() {
    ...
    // Add 10 edges with distinct source_doc_ids (starting from DocId(1) as DocId(0) is reserved sentinel)
    for i in 0..10 {
    ...
    // For each edge index 0..10: get_source_doc_id(idx) must be Some(DocId::new(idx + 1))
    for i in 0..10 {
        let idx = i as usize;
        let expected_doc = DocId::new(i + 1);
        assert_eq!(
...
async fn proof_source_doc_ids_consistent_after_multiple_compacts() {
    ...
    for i in 0..50 {
        let edge = Edge::new(EntityId::new(i), EntityId::new(i + 1), "rel1")
            .with_source_doc_id(DocId::new(i + 1));
