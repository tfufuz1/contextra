use super::group_commit::{GroupCommitRequest, PendingCommitQueue, WalQueueGuard};
use super::scan::{check_in_range, SstableScanMode};
use super::*;
...
impl StorageEngine for LsmStorage {
    ...
    fn put_if_absent<'a>(
        ...
        Box::pin(async move {
            ...
            // 1. Read-Your-Writes & Global Staging check across uncommitted transaction scopes.
            if let Some(is_insert) = self.tx_buffer.staged_status(key) {
                if is_insert {
                    return Ok(false);
                }
            } else if self.tx_buffer.is_key_staged_for_tx(tx_id, key) {
                return Ok(false);
            }
