...
impl CompactionEngine {
    ...
    async fn merge_sstables_inner(
        ...
        while let Some(item) = heap.pop() {
            ...
            let is_same_key = last_key.as_ref().map_or(false, |lk| lk == &item.key);
            if !is_same_key {
                floor_emitted = false;
            }

            // MVCC Floor Retention:
            // Retain all versions with seq > min_snapshot_seq.
            // Retain exactly one version with seq <= min_snapshot_seq (the floor version).
            // Drop any subsequent versions with seq <= min_snapshot_seq.
            let is_duplicate = if is_same_key {
