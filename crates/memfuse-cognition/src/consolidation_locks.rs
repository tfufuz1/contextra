...
//!
//! # Warum keine direkte Wiederverwendung von `memfuse_graph::session_dag::NodesGuard`?
//! `memfuse_graph::session_dag::NodesGuard` ist strikt an `SessionBranchTree`, `AgentStateNode`
//! (`NodeIdx` = `u64`) und `DagEdge` gebunden. In `memfuse-db` operieren Konsolidierung und Graph-Invalidierung
