...
def test_panic_translates_to_pyerr():
    ...
    with pytest.raises(RuntimeError) as exc_info:
        _contextra._trigger_panic_for_test("ADR-N04 test panic")
...
def test_panic_containment_in_subprocess():
    ...
    code = (
        "from contextra import _contextra\n"
        "try:\n"
        "    _contextra._trigger_panic_for_test('Subprocess panic containment')\n"
...
def test_db_and_collection_panic_poisoning():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        db = contextra.open(tmp_dir, dimension=128)
...
def test_release_wheel_panic_to_pyerr_boundary_isolation():
    ...
    Guarantees that:
    1. Direct module-level FFI panic hook `_contextra._trigger_panic_for_test` raises `PyRuntimeError`
    ...
    with pytest.raises(RuntimeError) as exc_mod:
        _contextra._trigger_panic_for_test("Release wheel module FFI panic")
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        db = contextra.open(tmp_dir, dimension=64)
        assert not db.is_poisoned
    ...
    subprocess_code = (
        "import contextra, tempfile, numpy as np, sys\n"
        "from contextra import _contextra\n"
        "try:\n"
        "    _contextra._trigger_panic_for_test('Release wheel subprocess panic')\n"
        "except RuntimeError as e:\n"
        "    assert 'Rust panic caught at FFI boundary' in str(e)\n"
