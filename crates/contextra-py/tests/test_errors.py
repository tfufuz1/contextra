...
def db(tmp_path):
    ...
    db = contextra.open(path, dimension=4)
    return db
...
def test_search_k_bounds(db):
    ...
    with pytest.raises((contextra.ContextraValueError, ValueError)):
        db.search(v, k=0)
    with pytest.raises((contextra.ContextraValueError, ValueError)):
        db.search(v, k=1001)
...
def test_invalid_distance_metric(tmp_path):
    ...
    with pytest.raises((contextra.ContextraValueError, ValueError)):
        contextra.open(path, dimension=4, distance_metric="invalid_metric")
...
def test_null_byte_validation(db, tmp_path):
    ...
    with pytest.raises(contextra.ContextraValueError) as excinfo:
        contextra.open(str(tmp_path) + "\0db", dimension=4)
    assert "cannot contain null bytes" in str(excinfo.value)
...
def test_numeric_id_validation(db):
    ...
    # Negative integer IDs must raise ContextraValueError
    with pytest.raises(contextra.ContextraValueError) as excinfo:
        db.insert(-1, v)
    ...
    # Under standard build, this raises ContextraValueError; under docid-128, it is accepted.
    ...
    except contextra.ContextraValueError as exc:
        assert "exceeds maximum allowed bound" in str(exc)

    # Overflowing integer IDs (> i128::MAX) must raise ContextraValueError
    overflow_id = 2**128 + 1
    with pytest.raises(contextra.ContextraValueError) as excinfo:
