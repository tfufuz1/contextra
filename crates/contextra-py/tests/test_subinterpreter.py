...
def test_worker_threads_per_initialization(tmp_path):
    """Verifies that CONTEXTRA_WORKER_THREADS is evaluated per module/interpreter initialization,
    ...
    # Run Python process 1 with CONTEXTRA_WORKER_THREADS=2
    code_1 = f"""import os, sys, contextra
db = contextra.open({db_path1!r}, dimension=16)
assert db.worker_threads == 2, f"Expected 2 worker threads, got {{db.worker_threads}}"
"""
    env1 = dict(os.environ, CONTEXTRA_WORKER_THREADS="2")
    ...
    # Run Python process 2 with CONTEXTRA_WORKER_THREADS=6
    code_2 = f"""import os, sys, contextra
db = contextra.open({db_path2!r}, dimension=16)
assert db.worker_threads == 6, f"Expected 6 worker threads, got {{db.worker_threads}}"
"""
    env2 = dict(os.environ, CONTEXTRA_WORKER_THREADS="6")
...
os.environ["CONTEXTRA_WORKER_THREADS"] = "3"
try:
    import contextra
    db = contextra.open({db_path1!r}, dimension=16)
...
sys.path.extend({sys.path!r})
os.environ["CONTEXTRA_WORKER_THREADS"] = "5"
try:
    import contextra
    db = contextra.open({db_path2!r}, dimension=16)
    print("LOADED_SUCCESS:" + str(db.worker_threads))
...
def test_subinterpreter_import_clean_rejection():
    """Verifies that importing contextra inside a subinterpreter is cleanly rejected by CPython
...
"""
    ...
    db = contextra.open(db_path, dimension=64)
...
def test_subinterpreter_attempt_during_concurrent_main_threads(tmp_path):
    ...
    db = contextra.open(db_path, dimension=32)
...
def test_single_interpreter_multiple_databases_performance(tmp_path):
    ...
    for i in range(5):
        ...
        db = contextra.open(db_path, dimension=16)
