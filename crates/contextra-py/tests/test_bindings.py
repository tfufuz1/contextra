...
def test_encryption_at_rest(tmp_path):
    ...
    db = contextra.open(path, dimension=4, encryption_passphrase=passphrase)
    ...
    db2 = contextra.open(path, dimension=4, encryption_passphrase=passphrase)
    ...
    with pytest.raises(Exception):
         contextra.open(path, dimension=4, encryption_passphrase="wrong-password")

def test_distance_metrics(db_path):
    # Euclidean
    db_l2 = contextra.open(db_path + "_l2", dimension=4, distance_metric="euclidean")
    col = db_l2.collection("test")
    v1 = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float32)
    col.insert("v1", v1)
    # Search with exact same vector, should have score 0.0 (distance) or 1.0 (similarity)
    # Actually HNSW usually returns distance. In contextra-db SearchResult it is called "score".
    # For Cosine it is 1.0 - cosine_dist.
    results = col.search(v1, k=1)
    assert results[0].id == "v1"

    # Dot Product
    db_dot = contextra.open(db_path + "_dot", dimension=4, distance_metric="dot")
    ...
    with pytest.raises(ValueError):
        contextra.open(db_path + "_invalid", dimension=4, distance_metric="invalid")

def test_version_and_repr(db_path):
    assert contextra.__version__ == "0.1.0"

    db = contextra.open(db_path, dimension=4, max_elements=5000)
...
def test_import():
    """contextra-Modul muss importierbar sein ohne Crash."""
    try:
        import contextra  # noqa: F401
    except ImportError:
...
def test_open_and_close(tmp_path):
    ...
    try:
        ...
        pytest.skip("maturin develop noch nicht ausgeführt")
    db = contextra.open(str(tmp_path))
    col = db.collection("test_col")
    assert col is not None, "collection() darf nicht None zurückgeben"


def test_search_fb_and_hybrid_search_fb(db_path):
    db = contextra.open(db_path, dimension=4)
    col = db.collection("fb_docs")
    v = np.array([0.1, 0.2, 0.3, 0.4], dtype=np.float32)
    col.insert("doc1", v, metadata={"text": "flatbuffers test"})

    fb_res = col.search_fb(v, k=1)
    assert isinstance(fb_res, bytes)
    assert len(fb_res) > 0

    fb_hybrid_res = col.hybrid_search_fb("flatbuffers", v, k=1)
    assert isinstance(fb_hybrid_res, bytes)
    assert len(fb_hybrid_res) > 0

    db_fb_res = db.search_fb(v, k=1)
    assert isinstance(db_fb_res, bytes)

    db_fb_hybrid_res = db.hybrid_search_fb("flatbuffers", v, k=1)
    assert isinstance(db_fb_hybrid_res, bytes)


def test_flatbuffer_search_docstrings_no_zero_copy_claim(db_path):
    db = contextra.open(db_path, dimension=4)
    col = db.collection("docstring_test")

    search_fb_doc = (col.search_fb.__doc__ or "").lower()
    hybrid_search_fb_doc = (col.hybrid_search_fb.__doc__ or "").lower()
    db_search_fb_doc = (db.search_fb.__doc__ or "").lower()
    db_hybrid_search_fb_doc = (db.hybrid_search_fb.__doc__ or "").lower()

    assert "zero-copy" not in search_fb_doc
    assert "zero copy" not in search_fb_doc
    assert "zero-copy" not in hybrid_search_fb_doc
    assert "zero copy" not in hybrid_search_fb_doc
    assert "zero-copy" not in db_search_fb_doc
    assert "zero copy" not in db_search_fb_doc
    assert "zero-copy" not in db_hybrid_search_fb_doc
    assert "zero copy" not in db_hybrid_search_fb_doc
