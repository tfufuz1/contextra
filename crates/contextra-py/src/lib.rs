...
use contextra_db::{Collection as ContextraCollection, Contextra, ContextraConfig};
...
// AI-TAG[FIX][PY-1][RESOLVED] Clamp CONTEXTRA_WORKER_THREADS to valid range [MIN_WORKER_THREADS, MAX_WORKER_THREADS] (TS: 2026-09-10T19:22:55Z) (SESSION: 0b2ff57d)
// Prevents Tokio Builder panic on worker_threads == 0 when CONTEXTRA_WORKER_THREADS=0 is set in environment.
...
// BEFUND: Resolved by decoupling `crates/contextra-py` into an independent workspace with its own `[profile.release]` setting `panic = "unwind"`.
...
// Siehe docs/decisions/ADR-064-contextra-py-separater-workspace-panic-strategie.md und ADR-N04 (PHASE0-T3)
...
fn doc_to_py(py: Python<'_>, d: contextra_db::Document) -> PyResult<PyDocument> {
...
fn contextra_err(e: contextra_core::ContextraError) -> PyErr {
...
// (Contextra vs Collection) share identical batch signatures.
...
mod tests {
    ...
    fn test_parse_worker_threads_env_clamping() {
        ...
        std::env::remove_var("CONTEXTRA_WORKER_THREADS");
    }
    ...
    fn test_py_err_mapping_all_error_kinds() {
        pyo3::prepare_freethreaded_python();
        Python::with_gil(|py| {
            let err_not_found = contextra_err(ContextraError::NotFound("doc1".into()));
            assert!(err_not_found.is_instance_of::<PyKeyError>(py));

            let err_invalid = contextra_err(ContextraError::InvalidInput("bad input".into()));
            assert!(err_invalid.is_instance_of::<PyValueError>(py));

            let err_policy = contextra_err(ContextraError::PolicyViolation("access denied".into()));
            assert!(err_policy.is_instance_of::<PyPermissionError>(py));

            let err_storage = contextra_err(ContextraError::Storage("io error".into()));
            assert!(err_storage.is_instance_of::<ContextraIOError>(py));

            let err_index = contextra_err(ContextraError::Index("hnsw error".into()));
            assert!(err_index.is_instance_of::<ContextraIndexError>(py));

            let err_crypto = contextra_err(ContextraError::Crypto("key error".into()));
            assert!(err_crypto.is_instance_of::<ContextraCryptoError>(py));

            let err_mem = contextra_err(ContextraError::MemoryBudgetExceeded {
                used_mb: 200,
                limit_mb: 100,
            });
            ...
            let err_internal = contextra_err(ContextraError::Internal("crash".into()));
            assert!(err_internal.is_instance_of::<ContextraInternalError>(py));
        });
    }
    ...
    fn test_contextra_err_attributes_set() -> PyResult<()> {
        pyo3::prepare_freethreaded_python();
        Python::with_gil(|py| {
            let py_err = contextra_err(ContextraError::NotFound("key123".into()));
