...
use memfuse_types::types::domain::{DocId, EntityId};
...
mod tests {
    ...
    fn test_graph_mutation_error_conversion() {
        ...
        let memfuse_err: MemFuseError = err.into();
        assert!(
            matches!(memfuse_err, MemFuseError::InvalidInput(ref msg) if msg.contains("Invalid role binding: role_0"))
        );
    }
}
