...
async fn test_list_tools() {
    ...
    assert!(tool_names.contains(&"contextra_search"));
    assert!(tool_names.contains(&"contextra_get"));
    assert!(tool_names.contains(&"contextra_insert"));
    assert!(tool_names.contains(&"contextra_forget"));
    assert!(tool_names.contains(&"contextra_collections"));
}
...
async fn test_search_missing_collection_returns_error() {
    // TESTZWECK: contextra_search ohne "collection"-Parameter → Fehler oder Default
    let (server, _tmp) = setup_app().await;
    let req = JsonRpcRequest {
        jsonrpc: "2.0".to_string(),
        id: Some(json!(101)),
    ...
    assert!(
        ...
        "Fehlende 'collection' bei contextra_search muss Fehler oder Fallback erzeugen: {val}"
    );
}

#[tokio::test]
async fn test_get_nonexistent_id_returns_controlled_response() {
    // TESTZWECK: contextra_get mit nicht-existierender ID → kein Panic
...
async fn test_insert_empty_text_returns_error() {
    // TESTZWECK: text="" bei contextra_insert → Fehler (leerer Text = kein Dokument)
    let (server, _tmp) = setup_app().await;
    let req = JsonRpcRequest {
        jsonrpc: "2.0".to_string(),
        id: Some(json!(103)),
    ...
    assert!(
        ...
        "Leerer 'text' bei contextra_insert muss Fehler erzeugen: {val}"
    );
}
...
async fn test_insert_missing_collection_field_returns_error() {
    // TESTZWECK: contextra_insert mit id+text aber ohne "collection" → Fehler oder Default
    let (server, _tmp) = setup_app().await;
    let req = JsonRpcRequest {
        jsonrpc: "2.0".to_string(),
        id: Some(json!(106)),
    ...
    assert!(
        response_is_error(&val) || val["result"].is_object(),
        "Fehlendes 'collection' bei contextra_insert muss Fehler oder Fallback erzeugen: {val}"
    );
}
...
async fn test_mcp_insert_multi_chunk_document() {
    ...
    // Retrieve chunk 0 via contextra_get
...
async fn test_prompt_injection_detection_and_content_provenance() {
    ...
    // 1. Check contextra_get response for provenance and injection flags
    let req_get = JsonRpcRequest {
        jsonrpc: "2.0".to_string(),
        id: Some(json!(31)),
        method: "tools/call".to_string(),
    ...
    assert_eq!(
        ...
        contextra_mcp::DEFAULT_REDACTION_PLACEHOLDER
    );

    // 2. Check contextra_search response for provenance and injection flags
...
async fn test_mcp_prompt_injection_quarantine_modes_integration() {
    use contextra_mcp::{PromptInjectionGuard, QuarantinePolicy, SecurityAuditLogger};
    ...
    // Fetch document via contextra_get in Escalate mode
    let req_get = JsonRpcRequest {
        jsonrpc: "2.0".to_string(),
        id: Some(json!(51)),
        method: "tools/call".to_string(),
...
async fn mcp_parse_error_returns_rpc_32700() {
    ...
    let err_resp =
        contextra_mcp::protocol::JsonRpcResponse::err(None, -32700, "Parse error: invalid json");
...
async fn test_stdio_transport_stability() {
    ...
    let bin_path = env!("CARGO_BIN_EXE_contextra-mcp-server");
    ...
    let mut child = tokio::process::Command::new(bin_path)
        ...
        .spawn()
        .expect("spawn contextra-mcp-server binary");
...
async fn test_mcp_boot_without_routing_config() {
    ...
    let router_config = contextra_mcp::RouterConfig::default();
    let routing = contextra_mcp::setup_routing(&db, &router_config)
        .await
...
async fn test_mcp_boot_with_routing_config() {
    ...
    let profile = contextra::router::SlmProfile::new(
        "test-profile",
    ...
    let router_config = contextra_mcp::RouterConfig {
...
async fn test_e2e_stdio_demo_flow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 2. Send contextra_insert request (matching README demo step 3)
    let req_insert = json!({
        "jsonrpc": "2.0",
        "id": 2,
        "method": "tools/call",
    ...
    // 3. Send contextra_search request (matching README demo step 4)
    let req_search = json!({
        "jsonrpc": "2.0",
        "id": 3,
        "method": "tools/call",
