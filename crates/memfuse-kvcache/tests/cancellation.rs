...
use memfuse_kvcache::{EvictionWorker, KvSegment, TenantIsolatedKvStore};
use std::sync::Arc;
use std::thread;

#[test]
fn test_cancellation_raii_guard_prevents_eviction_and_releases() -> Result<(), MemFuseError> {
...
#[test]
fn test_cancellation_eviction_worker_notification() -> Result<(), MemFuseError> {
...
#[test]
fn test_cancellation_transaction_rollback_cleans_up_staged_segments() -> Result<(), MemFuseError> {
    let store = TenantIsolatedKvStore::new();
...
#[test]
fn test_cancellation_concurrent_requests_guard_safety() -> Result<(), MemFuseError> {
    let store = Arc::new(TenantIsolatedKvStore::new());
