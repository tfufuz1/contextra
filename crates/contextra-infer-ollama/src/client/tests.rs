...
fn test_batch_size_consistency() {
    assert_eq!(
        MAX_BATCH_SIZE, 512,
        "Ollama MAX_BATCH_SIZE must default to 512"
    );
}
...
fn test_validate_model_name() {
    ...
    assert!(matches!(
        ...
        Err(ContextraError::PolicyViolation(_))
    ));
    assert!(matches!(
...
async fn test_chat_with_rag_streaming_invalid_json_returns_error() {
    ...
    match err {
        ContextraError::Serialization(msg) => {
            assert!(msg.contains("Failed to parse streaming JSON chunk"));
        }
        _ => panic!("Expected ContextraError::Serialization, got {:?}", err),
    }
}
...
async fn test_model_not_found_error_message() {
    ...
    match err {
        ...
        _ => panic!("Expected ContextraError::NotFound, got {:?}", err),
    }
}
...
async fn test_check_availability_offline_returns_io_error() {
    ...
    match err {
        ...
        _ => panic!("Expected ContextraError::Io, got {:?}", err),
    }
    assert!(!client.is_available().await);
}
...
// GATE    : cargo test -p contextra-ollama --test client
#[tokio::test]
...
async fn test_mock_server_connection_refused_error_classification() {
    ...
    assert!(matches!(err, ContextraError::Io(_)));
}
...
async fn test_list_models_http_error_and_invalid_json() {
    ...
    assert!(matches!(err_500, ContextraError::Storage(_)));
    ...
    assert!(matches!(err_json, ContextraError::Internal(_)));
}
...
async fn test_try_generate_mock_success_and_errors() {
    ...
    let err_400 = client
        ...
        .await
        .unwrap_err();
    assert!(matches!(err_400, ContextraError::InvalidInput(_)));

    let err_404 = client
        .try_generate("missing-gen-model", "prompt")
        .await
        .unwrap_err();
    assert!(matches!(err_404, ContextraError::NotFound(_)));
}
    ...
    for prompt in &malformed_prompts {
        ...
        if let Err(err) = res {
            assert!(
                ...
                "Expected ContextraError::Internal, got: {:?}",
                err
            );
        }
    }
