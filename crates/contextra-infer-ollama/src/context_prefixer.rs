...
// ZWECK: Contextra Contextual Chunk Prefixing — LLM-basierte Präfix-Generierung für Chunks
...
//! Contextra Contextual Chunk Prefixing: LLM-generierte Kontext-Präfixe für Chunks.
//!
//! Implementiert das Contextra Contextual-Chunk-Prefixing-Verfahren (angelehnt an etablierte Contextual-Retrieval-Methodik aus der Fachliteratur):
...
impl ContextPrefixEngine {
    ...
    ///
    /// Der Prompt folgt dem Contextra-Contextual-Chunk-Prefixing-Schema:
    /// "Describe this chunk in context of the full document (1-2 sentences)."
    ///
    /// # Fehler
    /// - `ContextraError::Storage` / `ContextraError::Io` wenn Ollama nicht erreichbar
    /// - `ContextraError::InvalidInput` wenn document oder chunk leer
    ...
    pub async fn generate_prefix_batch(
        ...
        full_document: &str,
        chunks: &[&str],
    ) -> Vec<Result<String, ContextraError>> {
