...
impl IsotonicCalibrator {
    ...
    /// Gibt den Zeitstempel (UNIX-Timestamp in Sekunden) der letzten Kalibrierung zurück.
    pub fn last_calibration_at(&self) -> Option<u64> {
        self.last_calibration_at
    }

    /// Gibt den gecachten Expected Calibration Error (ECE) zurück.
    ...
    /// Erzwingt ein sofortiges Rebuild des PAVA-Modells.
...
mod tests {
    ...
    fn test_debounced_rebuild_below_threshold_returns_old_model() {
        ...
        assert_eq!(prob_below_threshold, initial_prob);
    }
    ...
    fn test_force_rebuild_overrides_threshold() {
        ...
        assert_ne!(prob_after_forced, initial_prob);
    }
}
