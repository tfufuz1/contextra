...
impl ProvenanceRecord {
    /// Creates a ProvenanceRecord initialized from synthesized source DocIds.
...
/// Individual signal contribution record.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct SignalContribution {
    /// Raw unnormalized score from the underlying search engine.
    pub raw_score: f32,
    /// 1-based rank within the signal set.
    pub rank: u32,
    /// Computed RRF or score contribution.
    pub rrf_contribution: f32,
}

/// A scored search result with provenance tracking.
...
/// Configures signal priority order for metadata merging.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub enum MetadataMergePriority {
    /// Vector metadata is processed first. Order: Vector, Text, Graph.
    #[default]
    VectorFirst,
...
impl ProvenanceBuilder {
    /// Creates a new `ProvenanceBuilder` with parameter `rrf_k`.
...
pub fn weighted_reciprocal_rank_fusion_with_options(
    ...
    for cand in ranked_candidates {
        ...
        let mut signal_ranks: AHashMap<String, u32> = entry
            .signal_ranks
            .drain()
            .map(|(k, v): (SignalKey<'_>, u32)| (k.as_str().to_string(), v))
            .collect();
        ...
        let mut signal_contributions: AHashMap<String, SignalContribution> = entry
            .signal_contributions
            .drain()
            .map(|(k, v): (SignalKey<'_>, SignalContribution)| (k.as_str().to_string(), v))
            .collect();
...
pub fn score_normalized_fusion_with_options(
    ...
    for (_sig_name, set, _) in &active_sets {
