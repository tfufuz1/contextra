...
//! Loom-basierter Nebenläufigkeitsbeweis für Deferred Zeroize in `memfuse-security` (`crates/memfuse-crypto`).
//! Ausführung: RUSTFLAGS="--cfg loom" cargo test -p memfuse-security --test loom_kv_deferred_zeroize -- --nocapture
//!
...
mod loom_tests {
    ...
    // aus `crates/memfuse-crypto/src/kv_segment/store.rs` unter Verwendung von `loom::sync::RwLock`
