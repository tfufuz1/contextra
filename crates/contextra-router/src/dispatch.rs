...
fn split_endpoint(s: &str) -> Result<(String, Vec<String>)> {
    ...
    let program = iter
        .next()
        .ok_or_else(|| ContextraError::InvalidInput("Empty MCP endpoint".into()))?;
...
pub async fn dispatch_to_slm(decision: &RoutingDecision) -> Result<String> {
    ...
    if endpoint.is_empty() {
        return Err(ContextraError::InvalidInput("Empty MCP endpoint".into()));
    }
    ...
    let rpc_response: JsonRpcResponse = serde_json::from_str(&line)
        .map_err(|e| ContextraError::Internal(format!("Ungültige MCP JSON-RPC Antwort: {e}")))?;
