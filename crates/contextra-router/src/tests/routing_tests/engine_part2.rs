...
fn test_select_profile_from_chunks_empty_chunks_and_unmatched_community() {
    ...
    let err_low = select_profile_from_chunks(std::slice::from_ref(&profile), &[chunk_low_score]);
    assert!(
...
async fn test_select_profile_cascade_error_branches_and_fallback(
    ...
    let err_no_comm =
        router.select_profile_cascade(&[(chunk.clone(), Some(999))], &profiles, &mut calibration);
