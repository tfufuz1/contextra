...
//!
//! Ausführung: RUSTFLAGS="--cfg loom" cargo test -p contextra-store --test loom_group_commit --release
//!
...
use contextra_store::wal::{PreparedBatch, Wal, WalOp};
use std::sync::Arc;

struct GroupCommitRequest {
    _tx_id: u64,
    batch: PreparedBatch,
    sender: tokio::sync::oneshot::Sender<Result<(), ContextraError>>,
}
...
impl GroupCommitEngine {
    ...
    async fn commit(&self, tx_id: u64) -> Result<(), ContextraError> {
        ...
        if let Some(ref mut queue) = *queue_guard {
            ...
            rx.await
                .map_err(|_| ContextraError::Internal("Leader dropped commit channel".into()))?
        } else {
...
async fn verify_wal_hmac_chain(wal: &Wal) -> Result<usize, ContextraError> {
    let size = wal.size();
