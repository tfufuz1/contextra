...
//! a. Data-Block-Korruption -> `ContextraError::ChecksumMismatch` MUSS zurückkommen, NIEMALS ein stillschweigend falscher Wert.
...
proptest! {
    ...
    /// MUST trigger `ContextraError::ChecksumMismatch` when reading the affected key, NEVER a silently wrong value.
    #[test]
            ...
            if !layout.data_block_ranges.is_empty() {
                ...
                if block_end > payload_start {
                    ...
                    match reopened_res {
                        Err(e) => {
                            assert!(
                                matches!(e, ContextraError::ChecksumMismatch { .. }),
