...
use contextra_store::wal::FAIL_APPEND_FOR_TX;
...
/// we use a test-only fault injection hook (`FAIL_APPEND_FOR_TX` in `crates/contextra-store/src/wal.rs`).
/// When set to `tx.inner()`, any `append_batch()` call containing entries for `tx` fails with a
/// `ContextraError::Storage` I/O error, exercising the exact error path in `LsmStorage::commit()` where
