...
## Context
In `contextra-store`, group commit batching allows multiple concurrent transaction commit requests to be aggregated under a batch leader. Sequence allocation and HMAC state calculation occur sequentially during batch preparation under `commit_mutex`.
