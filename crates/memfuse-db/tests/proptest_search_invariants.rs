// FILE-CONTEXT
// STAND: 2026-09-15
// ZWECK: Property-based Tests für Retrieval- & Search-Invarianten in memfuse-db (RRF, BM25, End-to-End).
// INVARIANTEN:
//   PROP-1: RRF-Ergebnisse <= k
//   PROP-2: E2E Suche (Collection / MemFuse) <= k
...
use memfuse_db::fusion::{reciprocal_rank_fusion, weighted_reciprocal_rank_fusion};
use memfuse_db::{DistanceMetric, MemFuse, MemFuseConfig, SearchResult};
...
proptest! {
    ...
    fn prop_e2e_search_bounded_by_k(
        ...
        rt.block_on(async move {
            ...
            let db = MemFuse::open_with_config(tmp_dir.path(), config)
                .await
                .expect("Failed to open MemFuse DB");
