...
impl<S: StorageEngine, V: VectorIndex> Collection<S, V> {
    ...
    pub async fn relate_n_ary(
    ...
    ) -> Result<memfuse_graph::hyperedge::HyperEdgeId> {
        ...
        for &(doc_id_str, role_str) in participants {
            let entity_id = memfuse_core::EntityId::from_key(doc_id_str)?;
            let role_doc_id = memfuse_core::DocId::from_key(role_str)?;
            let role_id = memfuse_graph::hyperedge::RoleId::new(role_doc_id.inner() as u32);
        ...
        let db_tx = self.begin_transaction()?;
        let hyperedge_id = memfuse_graph::hyperedge::HyperEdgeId::new(db_tx.tx_id.inner());

        let role_bindings: Vec<memfuse_graph::hyperedge::RoleBinding> = resolved_participants
            .iter()
            .map(|&(_, role_id, entity_id)| {
                memfuse_graph::hyperedge::RoleBinding::new(role_id, entity_id)
            })
            .collect();
        ...
        let hyperedge = memfuse_graph::hyperedge::HyperEdge::new(
            hyperedge_id,
            memfuse_graph::csr::EdgeType::Default,
            role_bindings,
            1.0,
        )
        ...
        let hyperedge_key = format!(
            "{}{}",
            memfuse_graph::hyperedge::HYPEREDGE_PREFIX,
            hyperedge_id.inner()
        );
        ...
        for &(doc_id_str, _, entity_id) in &resolved_participants {
            let entity = memfuse_core::Entity::new(entity_id, doc_id_str, "Node");
...
mod tests {
    ...
    async fn test_db(dim: usize) -> (crate::MemFuse, TempDir) {
        ...
        let db = crate::MemFuse::open_with_config(tmp.path(), config)
            .await
            .expect("open db");
        (db, tmp)
    }
    ...
    async fn test_relate_n_ary_basic() {
        ...
        let e1 = memfuse_core::EntityId::from_key("doc-1").expect("entity_id");
    ...
    async fn test_relate_n_ary_insufficient_participants() {
        ...
        assert!(
            matches!(err, memfuse_core::MemFuseError::InvalidInput(ref msg) if msg.contains("at least 2 participants"))
        );
    }
}
