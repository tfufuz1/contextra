...
// NICHT-OFFENSICHTLICH: Lock-Hierarchie: collections (RwLock) -> kv_locks (Sharded Mutexes) -> embedder (RwLock).
...
//! 3. `Collection::kv_locks` (`KvKeyLocks`) / `Collection::embedder` (`parking_lot::RwLock`)
//!
//!
...
/// 2. `Collection::kv_locks` (`KvKeyLocks` sharded Mutexes):
