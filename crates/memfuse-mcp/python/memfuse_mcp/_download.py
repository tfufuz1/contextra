"""
Downloader and platform detection for memfuse-mcp-server release binaries.
...
def get_target_triple() -> Tuple[str, str]:
    ...
    raise RuntimeError(
        ...
        "Please set the MEMFUSE_MCP_BINARY environment variable pointing to a precompiled "
        "`memfuse-mcp-server` executable."
    )

def get_cache_dir(version: str) -> Path:
    """
    Determine local user cache directory for memfuse-mcp without external dependencies.
    ...
    cache_dir = base / "memfuse-mcp" / version
    cache_dir.mkdir(parents=True, exist_ok=True, mode=0o700)
    return cache_dir

def get_cached_binary_path(version: str) -> Path:
    """Return path to the cached memfuse-mcp-server binary for the given version."""
    bin_name = "memfuse-mcp-server.exe" if platform.system() == "Windows" else "memfuse-mcp-server"
...
def download_release_binary(version: str) -> Path:
    ...
    archive_filename = f"memfuse-mcp-server-{triple}.{ext}"
    base_release_url = f"https://github.com/tfufuz1/memfuse/releases/download/v{version}"
    ...
    target_bin_name = "memfuse-mcp-server.exe" if platform.system() == "Windows" else "memfuse-mcp-server"
    target_bin_path = cache_dir / target_bin_name

    headers = {"User-Agent": "memfuse-mcp-launcher"}

    # 1. Fetch SHA256SUMS
    sys.stderr.write(f"[memfuse-mcp] Fetching checksums from {checksums_url}...\n")
    sys.stderr.flush()
    ...
    sys.stderr.write(f"[memfuse-mcp] Downloading release binary from {archive_url}...\n")
    sys.stderr.flush()
    ...
    try:
        ...
        sys.stderr.write(f"[memfuse-mcp] Successfully installed binary to {target_bin_path}\n")
        sys.stderr.flush()
