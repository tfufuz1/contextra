...
pub async fn process_cloud_response(
    raw_response: &str,
    injection_guard: &crate::prompt_injection::PromptInjectionGuard,
...
fn evaluate_classification_result(classification: EgressClassification) -> Result<(), McpError> {
    match classification {
        ...
        EgressClassification::Block(BlockReason::PolicyDenied(reason)) => Err(
            McpError::invalid_params(format!("Egress policy violation: {reason}")),
        ),
        ...
        EgressClassification::Block(BlockReason::InternalError(err)) => Err(
            McpError::invalid_params(format!("Egress policy violation: {err}")),
        ),
...
mod tests {
    ...
    fn test_rehydration_multibyte_utf8_near_surrogate_no_panic() {
        ...
        // Test multi-byte UTF-8 characters (German Umlauts, CJK, Emoji) right next to [USER_ENTITY_ prefix
