...
// SIEHE AUCH:  crates/memfuse-mcp/src/egress_gateway.rs
...
mod tests {
    ...
    fn test_sliding_window_expiry_resets_volume() {
        ...
        // 3. At t=11s, original 800 bytes expire (> 10s window). 500 bytes should be Allowed!
    ...
    fn test_session_isolation() {
        ...
        // Bob's requests are completely isolated from Alice's volume
        assert_eq!(
            detector.record_and_check(s2.clone(), 900),
            BulkExfiltrationOutcome::Allow
        );

        // Alice exceeding limit does not block Bob
        assert_eq!(
    ...
    async fn test_concurrent_requests_prevent_limit_bypass_race() {
        ...
        // Spawn 10 concurrent tasks each recording 150 bytes (total 1500 bytes > 1000 limit)
        ...
        // Exactly 6 requests (6 * 150 = 900 <= 1000) are allowed, 4 requests are blocked.
    ...
    fn test_time_regression_fails_closed() {
        ...
        // Simulate clock regression (earlier instant than recorded entry)
