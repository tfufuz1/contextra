...
impl ContextManager {
    ...
    pub fn prepare_context(&self, mut chunks: Vec<ContextChunk>) -> Result<ContextWindow> {
        ...
        // Sort by relevance descending, then by doc_id ascending for deterministic tie-breaking // NOTE(H-17): ADR-Verweis nachführen — Spec §6.15 existiert nicht mehr
        chunks.sort_by(|a, b| {
            b.relevance
...
mod token_tests {
    ...
    fn test_context_chunk_try_from_search_result_invalid_id() {
        let sr = contextra_engine::SearchResult {
            id: "".to_string(),
            score: 0.9,
            metadata: None,
            matched_signals: vec![],
            provenance: Some(contextra_engine::ProvenanceRecord {
