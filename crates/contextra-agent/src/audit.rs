...
/// Append-only audit log backed by a Contextra collection.
...
impl<S: StorageEngine> AuditLog<S> {
    ...
    /// Enforces append-only immutability: returns `ContextraError::Conflict` if an entry for the step already exists.
