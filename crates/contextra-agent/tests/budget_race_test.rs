...
// ZWECK: Test token budget concurrency and RMW race condition in contextra-agent workflow steps.
...
async fn setup_env(
    max_tokens: usize,
) -> Result<(OrchestratorEngine, Arc<Contextra>, AgentContext, TempDir)> {
        ...
        for _ in 0..num_workers {
            ...
            handles.push(tokio::spawn(async move {
                ...
                if reserved {
                    ...
                    let db = Arc::new(
                        Contextra::open_with_config(dummy_tmp.path(), config)
                            .await
                            .unwrap(),
                    );
