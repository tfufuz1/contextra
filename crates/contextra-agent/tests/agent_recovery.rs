...
async fn setup_env() -> (Arc<Contextra>, TempDir) {
...
async fn test_agent_error_handling() {
    ...
    assert_eq!(ctx.status, contextra_agent::context::AgentStatus::Failed);

    // Verify failure logged in audit trail
    let audit_log = contextra_agent::audit::AuditLog::new(state_col);
...
async fn test_agent_audit_log_immutable() {
    ...
    let audit_log_instance = contextra_agent::audit::AuditLog::new(state_col);
