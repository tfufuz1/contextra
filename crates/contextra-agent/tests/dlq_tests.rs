...
async fn test_tool_timeout_creates_dead_letter() -> Result<()> {
    ...
    match res {
        ...
        _ => panic!("Expected ContextraError::Timeout, got {:?}", res),
    }
...
async fn test_concurrent_allocate_tx_uniqueness() -> Result<()> {
    use contextra_agent::DeadLetterQueue;
    ...
    impl AgentTool for FlakeyTool {
        ...
        fn execute<'a>(
            ...
            Box::pin(async move {
                ...
                if n == 0 {
                    Err(ContextraError::Internal("transient failure".into()))
                } else {
                    Ok(StepResult {
