...
mit der zugrundeliegenden `contextra-db` Collection.
...
- Keine Pfad-Trenner (`/`, `\`)
Verstöße führen zu sofortigem Fehler (`ContextraError::Validation`).
...
// ✅ KORREKT — Alle Tools MÜSSEN den globalen `ContextraError` verwenden.
```
...
- **Erlaubte Imports**: `contextra-core` (L0), `contextra-store` (L1), `contextra-db` (L2)
- **Verbotene Imports**: `contextra-mcp` (L4)
- **Genutzt von**: `contextra-mcp`, `contextra-router`
