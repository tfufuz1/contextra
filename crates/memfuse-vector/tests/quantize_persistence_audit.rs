...
use memfuse_vector::diskann::{DiskAnnConfig, DiskAnnFallbackPolicy, DiskAnnIndex};
use memfuse_vector::hnsw::{HnswConfig, HnswIndex};
use memfuse_vector::persistence::MmapIndex;
...
fn test_sq8_kendall_tau_rank_correlation() {
    ...
    for _ in 0..num_queries {
        ...
        let mut f32_dists_u: Vec<(DocId, f32)> = uniform_vectors
            ...
            .enumerate()
            .map(|(idx, v)| {
                let d = memfuse_vector::distance::cosine_distance_scalar(&q_uniform, v);
        ...
        let mut f32_dists_s: Vec<(DocId, f32)> = skewed_vectors
            ...
            .enumerate()
            .map(|(idx, v)| {
                let d = memfuse_vector::distance::cosine_distance_scalar(&q_skewed, v);
                (DocId::new((idx + 1) as u64), d)
            })
            .collect();
