// TEST-SUITE: Compound Split Recall Impact Benchmark (contextra-index / contextra-db)
...
async fn test_compound_split_recall_impact_evaluation() {
    ...
    let config_actual = ContextraConfig {
        dimension: dim,
        max_elements: 100,
        ..Default::default()
    };
    let db_actual = Contextra::open_with_config(dir_actual.path(), config_actual)
        .await
        .unwrap();
    ...
    let config_ref = ContextraConfig {
        dimension: dim,
        max_elements: 100,
        ..Default::default()
    };
    let db_ref = Contextra::open_with_config(dir_ref.path(), config_ref)
        .await
        .unwrap();
