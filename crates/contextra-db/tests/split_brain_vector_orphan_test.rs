...
/// b) On `Contextra::open_with_config` restart, `repair_on_open()` re-syncs HNSW with LSM storage
...
async fn test_vector_deleted_process_killed_before_document_tombstone() {
    ...
    let db = Contextra::open_with_config(&dir_path, config.clone())
        .await
        .expect("open db");
    ...
    let db_reopened = Contextra::open_with_config(&dir_path, config)
        .await
        .expect("reopen db");
...
async fn test_duplicate_insert_transaction_replay_no_double_vector_entry() {
    ...
    let db_reopened = Contextra::open_with_config(&dir_path, config.clone())
        .await
        .expect("reopen db");
    let col_reopened = db_reopened
...
proptest! {
    ...
    fn prop_no_dangling_search_result_under_random_insert_delete_sequence(ops in op_strategy()) {
        ...
        rt.block_on(async move {
            ...
            let db = Arc::new(Contextra::open_with_config(tmp.path(), config).await.expect("open db"));
