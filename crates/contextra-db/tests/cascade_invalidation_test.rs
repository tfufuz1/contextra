...
async fn test_e2e_supersedes_cascade_invalidates_graph_edges() {
    ...
    let db = Contextra::open(dir.path()).await.unwrap();
    ...
    col.insert(
        "doc3",
        &dummy_emb,
        Some(serde_json::json!({"text": "separate unrelated fact"})),
    )
    .await
    .unwrap();
    ...
    GraphIndex::add_entity(
        graph.as_ref(),
        tx1,
        Entity::new(node1, "Entity1", "Concept"),
    )
    .await
    .unwrap();
    GraphIndex::add_entity(
        graph.as_ref(),
        tx1,
        Entity::new(node2, "Entity2", "Concept"),
    )
    .await
    .unwrap();
    GraphIndex::add_entity(
        graph.as_ref(),
        tx1,
        Entity::new(node3, "Entity3", "Concept"),
    )
    .await
    .unwrap();
    ...
    GraphIndex::add_edge(graph.as_ref(), tx1, edge_doc1)
        .await
        .unwrap();
    GraphIndex::add_edge(graph.as_ref(), tx1, edge_doc3)
        .await
        .unwrap();
    ...
    assert_eq!(
        paths_before.len(),
        2,
        "Expected 2 paths (node1->node2 and node1->node2->node3) before invalidation"
    );
    ...
    assert!(
        ...
        "Paths originating from node1 via doc1 edge must be empty after cascade invalidation"
    );
    ...
    assert_eq!(
        paths_node2.len(),
        1,
        "Edge derived from doc3 must remain active"
    );

    // Calling link_memories again for doc1 should be idempotent
    let second_link_res = col
        .link_memories(doc2_id, doc1_id, LinkRelation::Supersedes)
        .await;
    assert!(
        second_link_res.is_ok(),
        "Idempotent link_memories call must succeed without error"
    );
}
