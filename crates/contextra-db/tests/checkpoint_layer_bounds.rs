...
// ZIEL: contextra-checkpoint -> contextra-db (Fork + Diverge + Merge)
//
// Dieser Test verifiziert die Zusammenarbeit zwischen dem PersistentCheckpointStore
// und der Contextra-DB Facade. Er simuliert einen "Fork", indem er eine neue
...
async fn test_layer_001_fork_diverge_merge() {
    ...
    let _cp_v1;
    {
        ...
        let storage = Arc::new(
            contextra_store::LsmStorage::new(lsm_config)
                .await
                .expect("storage"),
        );
        let cp_manager =
