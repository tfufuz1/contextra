...
// INVARIANTEN: crates/contextra-db/src/lib.rs MUST NOT contain `#![allow(deprecated)]`.
// STAND: TS:2026-09-05T00:00:00Z (SESSION: 0dcb9f3b)

#[test]
fn test_no_blanket_allow_deprecated_in_lib_rs() {
    let manifest_dir =
        std::env::var("CARGO_MANIFEST_DIR").unwrap_or_else(|_| "crates/contextra-db".to_string());
        ...
        if trimmed.contains("#![allow(deprecated)]") || trimmed.contains("#![allow(deprecated,") {
            panic!(
                "Regression failure: Crate-level blanket deprecation suppression found in {}:{}!\n\
                 Line {}: {}\n\
                 Rule: Crate-wide `#![allow(deprecated)]` is strictly forbidden in contextra-db.\n\
