...
use contextra_ports::{StorageEngine, StorageRead};
use contextra_types::TenantId;
use serde_json::json;
use tempfile::TempDir;

/// Helper: Öffnet eine frische Contextra-Instanz mit der angegebenen Dimension.
async fn setup_db(dim: usize) -> (Contextra, TempDir) {
...
use contextra_types::TxId;
    ...
    let storage = contextra_store::LsmStorage::new(config)
        .await
...
async fn test_2pc_recovery_after_crash() {
    ...
    {
        let db = Contextra::open_with_config(tmp.path(), config.clone())
            .await
            .expect("open db");
