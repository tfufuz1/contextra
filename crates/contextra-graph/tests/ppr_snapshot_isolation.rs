...
async fn test_ppr_snapshot_isolation_temporal_visibility() {
    ...
    graph
        .add_entity(tx10, Entity::new(node_a, "node_a", "Node"))
        .await
        .unwrap();
    graph
        .add_entity(tx10, Entity::new(node_b, "node_b", "Node"))
        .await
        .unwrap();
    GraphIndex::add_edge(
        &graph,
        tx10,
        Edge::new(node_a, node_b, "rel").with_weight(1.0),
    )
    .await
    .unwrap();
    ...
    graph
        .add_entity(tx20, Entity::new(node_c, "node_c", "Node"))
        .await
        .unwrap();
    GraphIndex::add_edge(
        &graph,
        tx20,
        Edge::new(node_a, node_c, "rel").with_weight(1.0),
    )
    .await
    .unwrap();
    ...
    let res_seq10 = graph
        .personalized_page_rank_at(&[node_a], &cfg, 10)
        .await
        .unwrap();
    let eids_seq10: Vec<_> = res_seq10.iter().map(|(id, _)| *id).collect();
    assert!(
        eids_seq10.contains(&node_b),
        "node_b must be present at seq 10"
    );
    assert!(
        !eids_seq10.contains(&node_c),
        "node_c must NOT be present at seq 10"
    );

    // Snapshot at seq 20: both B and C should be visible from A
    let res_seq20 = graph
        .personalized_page_rank_at(&[node_a], &cfg, 20)
        .await
        .unwrap();
    let eids_seq20: Vec<_> = res_seq20.iter().map(|(id, _)| *id).collect();
    assert!(
        eids_seq20.contains(&node_b),
        "node_b must be present at seq 20"
    );
    assert!(
        eids_seq20.contains(&node_c),
        "node_c must be present at seq 20"
    );
}
...
async fn test_ppr_snapshot_isolation_matches_live_at_latest_seq() {
    ...
    graph
        .add_entity(tx100, Entity::new(node_a, "node_a", "Node"))
        .await
        .unwrap();
    graph
        .add_entity(tx100, Entity::new(node_b, "node_b", "Node"))
        .await
        .unwrap();
    graph
        .add_entity(tx100, Entity::new(node_c, "node_c", "Node"))
        .await
        .unwrap();
    ...
    GraphIndex::add_edge(
        &graph,
        tx100,
        Edge::new(node_a, node_b, "rel").with_weight(0.8),
    )
    .await
    .unwrap();
    GraphIndex::add_edge(
        &graph,
        tx100,
        Edge::new(node_b, node_c, "rel").with_weight(0.5),
    )
    .await
    .unwrap();
    ...
    let snap_res = graph
        .personalized_page_rank_at(&[node_a], &cfg, 100)
        .await
        .unwrap();
    ...
    assert_eq!(
        live_res.len(),
        snap_res.len(),
        "Live and snapshot result count must match"
    );
    ...
    for ((live_id, live_score), (snap_id, snap_score)) in live_res.iter().zip(snap_res.iter()) {
        ...
        assert!(
            (live_score - snap_score).abs() < 1e-5,
            "Scores must match within tolerance: live={live_score}, snap={snap_score}"
        );
    }
}
...
async fn test_ppr_snapshot_isolation_tombstone_and_validity_range() {
    ...
    assert!(
        !ids_seq20.contains(&node_b),
        "node_b must NOT be present at seq 20 after edge validity expiration"
    );
}
...
async fn test_ppr_snapshot_isolation_hyperedge_visibility() {
    ...
    graph
        ...
        .unwrap();
    graph
        .add_entity(tx10, Entity::new(node_c, "node_c", "Node"))
        .await
        .unwrap();
    graph.commit(tx10).await.unwrap();
    ...
    assert!(
        !ids_seq10.contains(&node_b),
        "node_b must NOT be reachable via hyperedge at seq 10"
    );
    assert!(
        !ids_seq10.contains(&node_c),
        "node_c must NOT be reachable via hyperedge at seq 10"
    );
    ...
    assert!(
        ids_seq20.contains(&node_b),
        "node_b must be reachable via hyperedge at seq 20"
    );
    assert!(
        ids_seq20.contains(&node_c),
        "node_c must be reachable via hyperedge at seq 20"
    );
}
...
async fn test_ppr_snapshot_isolation_empty_seeds_and_determinism() {
    ...
    let empty_res = graph
        .personalized_page_rank_at(&[], &cfg, 100)
        .await
        .unwrap();
    assert!(
        empty_res.is_empty(),
        "Empty seed list must yield empty results"
    );
    ...
    let tx10 = TxId::new(10);
    graph
        .add_entity(tx10, Entity::new(node_a, "a", "N"))
        .await
        .unwrap();
    graph
        .add_entity(tx10, Entity::new(node_b, "b", "N"))
        .await
        .unwrap();
    GraphIndex::add_edge(
        &graph,
    ...
    assert_eq!(
        run1, run2,
        "Repeated snapshot PPR queries must be bit-identical"
    );
}
...
async fn test_ppr_snapshot_isolation_algorithms_parity() {
    ...
    graph
        ...
        .unwrap();
    graph
        .add_entity(tx10, Entity::new(node_c, "c", "N"))
        .await
        .unwrap();
    ...
    GraphIndex::add_edge(
        &graph,
        tx10,
        Edge::new(node_b, node_c, "rel").with_weight(0.6),
    )
    .await
    .unwrap();
    graph.commit(tx10).await.unwrap();
    ...
    let fp_res = graph
        .personalized_page_rank_at(&[node_a], &cfg_fp, 10)
        .await
        .unwrap();
    let dense_res = graph
        .personalized_page_rank_at(&[node_a], &cfg_dense, 10)
        .await
        .unwrap();
    ...
    assert_eq!(
        fp_ids, dense_ids,
        "ForwardPush and DensePowerIteration must produce the same candidate ordering on snapshot"
    );
}
