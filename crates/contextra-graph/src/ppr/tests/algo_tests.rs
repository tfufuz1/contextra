...
async fn test_ppr_tl_hfd_dispatch_valid() {
    ...
    graph
        .add_entity(tx, Entity::new(e1, "Node1", "TypeA"))
        .await
        .unwrap();
    graph
        .add_entity(tx, Entity::new(e2, "Node2", "TypeA"))
        .await
        .unwrap();
    graph
        .add_entity(tx, Entity::new(e3, "Node3", "TypeA"))
        .await
        .unwrap();
    ...
    assert!(
        !results.is_empty(),
        "TL-HFD dispatch must return non-empty PPR results"
    );

    for (entity, score) in &results {
        assert!(
            score.is_finite() && *score >= 0.0,
            "Entity {:?} score {} must be finite and non-negative",
            entity,
            score
        );
    }
...
async fn test_ppr_shadow_mode_tl_hfd_parity() {
    ...
    assert_eq!(
        fp_results.len(),
        shadow_results.len(),
        "ShadowModeTlHfd must return same result length as ForwardPush"
    );
    for ((id_fp, score_fp), (id_sh, score_sh)) in fp_results.iter().zip(shadow_results.iter()) {
        assert_eq!(
            id_fp, id_sh,
            "Entity IDs must match between ForwardPush and ShadowModeTlHfd"
        );
        assert_eq!(
            score_fp.to_bits(),
            score_sh.to_bits(),
            "Scores must be bit-identical between ForwardPush and ShadowModeTlHfd"
        );
    }
}
