...
fn tensor_to_payload(tensor: &Tensor) -> Result<TensorPayload, MemFuseError> {
    let shape = tensor.dims().to_vec();
    let cpu_tensor = tensor
        .to_device(&Device::Cpu)
        .map_err(|e| MemFuseError::Internal(format!("Failed to transfer tensor to CPU: {e}")))?;

    match tensor.dtype() {
        DType::F32 => {
            let flat = cpu_tensor
                .flatten_all()
                .map_err(|e| MemFuseError::Internal(format!("Failed to flatten tensor: {e}")))?;
            let floats = flat.to_vec1::<f32>().map_err(|e| {
                MemFuseError::Internal(format!("Failed to extract f32 vector: {e}"))
            })?;
        ...
        DType::F16 => {
            ...
            let f16s = flat.to_vec1::<f16>().map_err(|e| {
                MemFuseError::Internal(format!("Failed to extract f16 vector: {e}"))
            })?;
        ...
        other => Err(MemFuseError::capability_unsupported(
...
fn payload_to_tensor(payload: &TensorPayload, device: &Device) -> Result<Tensor, MemFuseError> {
    match payload.dtype.as_str() {
        "f32" => {
            ...
            let tensor = Tensor::from_vec(floats, payload.shape.as_slice(), &Device::Cpu)
                .map_err(|e| MemFuseError::Internal(format!("Failed to build tensor: {e}")))?;
            tensor
                .to_device(device)
                .map_err(|e| MemFuseError::Internal(format!("Failed to move tensor: {e}")))
        }
        "f16" => {
...
impl KvState {
    ...
    pub fn truncate(&mut self, len: usize) -> Result<(), MemFuseError> {
        if len >= self.pos {
            return Ok(());
        }
    ...
    pub fn export_block(&self, range: Range<usize>) -> Result<KvBlock, MemFuseError> {
        ...
        let encoded = bincode::serialize(&payload).map_err(|e| {
            MemFuseError::Internal(format!("Failed to serialize KvState block payload: {e}"))
        })?;
        ...
        if self.layers.is_empty() {
            ...
            for (layer, lp) in self.layers.iter_mut().zip(payload.layers) {
                ...
                layer.k = Tensor::cat(&[&layer.k, &imp_k], dim_k).map_err(|e| {
                    MemFuseError::Internal(format!("Failed to concatenate key tensors: {e}"))
                })?;
                layer.v = Tensor::cat(&[&layer.v, &imp_v], dim_v).map_err(|e| {
                    MemFuseError::Internal(format!("Failed to concatenate value tensors: {e}"))
                })?;
            }
            self.pos += payload.seq_len;
        }
    ...
    pub fn import_block_at(&mut self, block: &KvBlock, at: usize) -> Result<(), MemFuseError> {
        if at == self.pos {
