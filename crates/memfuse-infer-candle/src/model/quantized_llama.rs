...
fn repeat_kv(x: Tensor, n_rep: usize) -> Result<Tensor, MemFuseError> {
    if n_rep == 1 {
        Ok(x)
    } else {
        let (b_sz, n_kv_head, seq_len, head_dim) = x
            .dims4()
            .map_err(|e| MemFuseError::Internal(format!("Failed to get dims4: {e}")))?;
        let x = x
            .unsqueeze(2)
            .map_err(|e| MemFuseError::Internal(format!("Failed to unsqueeze tensor: {e}")))?
            .expand((b_sz, n_kv_head, n_rep, seq_len, head_dim))
            .map_err(|e| MemFuseError::Internal(format!("Failed to expand tensor: {e}")))?
            .reshape((b_sz, n_kv_head * n_rep, seq_len, head_dim))
            .map_err(|e| MemFuseError::Internal(format!("Failed to reshape tensor: {e}")))?;
        Ok(x)
    }
}
...
fn precomput_freqs_cis(
    ...
    freq_base: f32,
    device: &Device,
) -> Result<(Tensor, Tensor), MemFuseError> {
    let theta: Vec<_> = (0..head_dim)
        .step_by(2)
        .map(|i| 1f32 / freq_base.powf(i as f32 / head_dim as f32))
        .collect();
    let theta = Tensor::new(theta.as_slice(), device)
        .map_err(|e| MemFuseError::Internal(format!("theta tensor creation error: {e}")))?;
    let idx_theta = Tensor::arange(0u32, MAX_SEQ_LEN as u32, device)
        .map_err(|e| MemFuseError::Internal(format!("arange error: {e}")))?
        .to_dtype(DType::F32)
        .map_err(|e| MemFuseError::Internal(format!("to_dtype error: {e}")))?
        .reshape((MAX_SEQ_LEN, 1))
        .map_err(|e| MemFuseError::Internal(format!("reshape error: {e}")))?
        .matmul(
            &theta
                .reshape((1, theta.elem_count()))
                .map_err(|e| MemFuseError::Internal(format!("theta reshape error: {e}")))?,
        )
        .map_err(|e| MemFuseError::Internal(format!("matmul error: {e}")))?;
    let cos = idx_theta
        .cos()
        .map_err(|e| MemFuseError::Internal(format!("cos error: {e}")))?;
    let sin = idx_theta
        .sin()
        .map_err(|e| MemFuseError::Internal(format!("sin error: {e}")))?;
    Ok((cos, sin))
}
...
fn build_causal_mask(
    ...
    Tensor::from_slice(&mask, (seq_len, seq_len + index_pos), device)
        .map_err(|e| MemFuseError::Internal(format!("Failed to create causal mask tensor: {e}")))
}
...
impl QuantizedMatMul {
    fn from_qtensor(qtensor: QTensor) -> Result<Self, MemFuseError> {
        let inner = QMatMul::from_qtensor(qtensor)
            .map_err(|e| MemFuseError::Internal(format!("Failed to build QMatMul: {e}")))?;
        let span = tracing::span!(tracing::Level::TRACE, "qmatmul");
        Ok(Self { inner, span })
    }

    fn forward(&self, xs: &Tensor) -> Result<Tensor, MemFuseError> {
        let _enter = self.span.enter();
        self.inner
            .forward(xs)
            .map_err(|e| MemFuseError::Internal(format!("QMatMul forward error: {e}")))
    }
}
...
impl Mlp {
    fn forward(&self, xs: &Tensor) -> Result<Tensor, MemFuseError> {
        ...
        let silu_w1 = candle_nn::ops::silu(&w1)
            .map_err(|e| MemFuseError::Internal(format!("SiLU error: {e}")))?;
        let mul = (silu_w1 * w3)
            .map_err(|e| MemFuseError::Internal(format!("Tensor multiplication error: {e}")))?;
        ...
        match self {
            Self::MoE {
                ...
                let (b_size, seq_len, hidden_dim) = xs
                    .dims3()
                    .map_err(|e| MemFuseError::Internal(format!("Failed dims3: {e}")))?;
                let xs_reshaped = xs
                    .reshape(((), hidden_dim))
                    .map_err(|e| MemFuseError::Internal(format!("Failed reshape: {e}")))?;
                let router_logits = feed_forward_gate_inp.forward(&xs_reshaped)?;
                let routing_weights = candle_nn::ops::softmax_last_dim(&router_logits)
                    .map_err(|e| MemFuseError::Internal(format!("Softmax error: {e}")))?;

                let routing_weights_vec = routing_weights
                    .to_dtype(DType::F32)
                    .map_err(|e| MemFuseError::Internal(format!("to_dtype error: {e}")))?
                    .to_vec2::<f32>()
                    .map_err(|e| MemFuseError::Internal(format!("to_vec2 error: {e}")))?;
                ...
                let mut ys = xs_reshaped
                    .zeros_like()
                    .map_err(|e| MemFuseError::Internal(format!("zeros_like error: {e}")))?;
                for (expert_idx, expert_layer) in experts.iter().enumerate() {
                    ...
                    let top_x_tensor = Tensor::new(top_x_indices.as_slice(), xs.device())
                        .map_err(|e| MemFuseError::Internal(format!("Tensor::new error: {e}")))?;
                    ...
                    let current_state = xs_reshaped
                        ...
                        .map_err(|e| MemFuseError::Internal(format!("index_select error: {e}")))?
                        .reshape(((), hidden_dim))
                        .map_err(|e| MemFuseError::Internal(format!("reshape error: {e}")))?;
                    let current_hidden_states = expert_layer.forward(&current_state)?;
                    let current_hidden_states = current_hidden_states
                        .broadcast_mul(&selected_rws_tensor)
                        .map_err(|e| MemFuseError::Internal(format!("broadcast_mul error: {e}")))?;
                    ys = ys
                        .index_add(&top_x_tensor, &current_hidden_states, 0)
                        .map_err(|e| MemFuseError::Internal(format!("index_add error: {e}")))?;
                }
...
impl LayerWeights {
    fn apply_rotary_emb(&self, x: &Tensor, index_pos: usize) -> Result<Tensor, MemFuseError> {
        let _enter = self.span_rot.enter();
        let (_b_sz, _n_head, seq_len, _n_embd) = x
            .dims4()
            .map_err(|e| MemFuseError::Internal(format!("Failed dims4 in RoPE: {e}")))?;
        let cos = self
            .cos
            .narrow(0, index_pos, seq_len)
            .map_err(|e| MemFuseError::Internal(format!("Narrow cos error: {e}")))?;
        let sin = self
            .sin
            .narrow(0, index_pos, seq_len)
            .map_err(|e| MemFuseError::Internal(format!("Narrow sin error: {e}")))?;
        let x_cont = x
            .contiguous()
            .map_err(|e| MemFuseError::Internal(format!("Contiguous error: {e}")))?;
        if self.rope_is_neox {
            candle_nn::rotary_emb::rope(&x_cont, &cos, &sin)
                .map_err(|e| MemFuseError::Internal(format!("RoPE error: {e}")))
        } else {
            candle_nn::rotary_emb::rope_i(&x_cont, &cos, &sin)
                .map_err(|e| MemFuseError::Internal(format!("RoPE_i error: {e}")))
        }
    }
    ...
    fn forward_attn(
        ...
        let (b_sz, seq_len, _n_embd) = x
            .dims3()
            .map_err(|e| MemFuseError::Internal(format!("dims3 error: {e}")))?;
        ...
        let q = q
            ...
            .map_err(|e| MemFuseError::Internal(format!("q reshape error: {e}")))?
            .transpose(1, 2)
            .map_err(|e| MemFuseError::Internal(format!("q transpose error: {e}")))?;
        let k = k
            .reshape((b_sz, seq_len, self.n_kv_head, self.head_dim))
            .map_err(|e| MemFuseError::Internal(format!("k reshape error: {e}")))?
            .transpose(1, 2)
            .map_err(|e| MemFuseError::Internal(format!("k transpose error: {e}")))?;
        let v = v
            .reshape((b_sz, seq_len, self.n_kv_head, self.head_dim))
            .map_err(|e| MemFuseError::Internal(format!("v reshape error: {e}")))?
            .transpose(1, 2)
            .map_err(|e| MemFuseError::Internal(format!("v transpose error: {e}")))?
            .contiguous()
            .map_err(|e| MemFuseError::Internal(format!("v contiguous error: {e}")))?;
            ...
            Some((k_cache, v_cache)) => {
                if index_pos == 0 {
                    ...
                    let k_cat = Tensor::cat(&[k_cache, &k], 2)
                        .map_err(|e| MemFuseError::Internal(format!("k cat error: {e}")))?;
                    let v_cat = Tensor::cat(&[v_cache, &v], 2)
                        .map_err(|e| MemFuseError::Internal(format!("v cat error: {e}")))?;
                    (k_cat, v_cat)
                }
            }
        };
        ...
        let att = (q
            .matmul(&kt)
            .map_err(|e| MemFuseError::Internal(format!("q @ k error: {e}")))?
            / (self.head_dim as f64).sqrt())
        .map_err(|e| MemFuseError::Internal(format!("scale att error: {e}")))?;
        ...
        let att = candle_nn::ops::softmax_last_dim(&att)
            .map_err(|e| MemFuseError::Internal(format!("softmax att error: {e}")))?;
        let y = att
            .matmul(&v_rep)
            .map_err(|e| MemFuseError::Internal(format!("att @ v error: {e}")))?;
        let y = y
            .transpose(1, 2)
            .map_err(|e| MemFuseError::Internal(format!("y transpose error: {e}")))?
            .reshape((b_sz, seq_len, self.n_head * self.head_dim))
            .map_err(|e| MemFuseError::Internal(format!("y reshape error: {e}")))?;
...
impl ModelWeights {
    pub fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        let tok_embeddings = ct
            ...
            .map_err(|e| MemFuseError::Internal(format!("Failed token_embd.weight: {e}")))?;
        let tok_embeddings = tok_embeddings
            .dequantize(device)
            .map_err(|e| MemFuseError::Internal(format!("Failed dequantize embeddings: {e}")))?;
        let norm = ct
            .tensor(reader, "output_norm.weight", device)
            .map_err(|e| MemFuseError::Internal(format!("Failed output_norm.weight: {e}")))?;
        let output = ct
            .tensor(reader, "output.weight", device)
            .map_err(|e| MemFuseError::Internal(format!("Failed output.weight: {e}")))?;

        let mut layers = Vec::with_capacity(block_count);
        let head_dim = embedding_length / head_count;

        let neg_inf = Tensor::new(f32::NEG_INFINITY, device)
            .map_err(|e| MemFuseError::Internal(format!("neg_inf tensor creation failed: {e}")))?;
    ...
    pub fn export_kv_state(&self) -> Result<KvState, MemFuseError> {
    ...
    pub fn import_kv_state(&mut self, state: &KvState) -> Result<(), MemFuseError> {
    ...
    pub fn truncate_kv_cache(&mut self, pos: usize) -> Result<(), MemFuseError> {
                ...
                if pos == 0 {
                    ...
                    if pos < k_len {
                        let new_k = k.narrow(2, 0, pos).map_err(|e| {
                            MemFuseError::Internal(format!("Failed to truncate key tensor: {e}"))
                        })?;
                        let new_v = v.narrow(2, 0, pos).map_err(|e| {
                            MemFuseError::Internal(format!("Failed to truncate value tensor: {e}"))
                        })?;
    ...
    pub fn forward(&mut self, x: &Tensor, index_pos: usize) -> Result<Tensor, MemFuseError> {
        let (_b_sz, seq_len) = x
            .dims2()
            .map_err(|e| MemFuseError::Internal(format!("dims2 error: {e}")))?;
        ...
        let mut layer_in = self
            .tok_embeddings
            .forward(x)
            .map_err(|e| MemFuseError::Internal(format!("tok_embeddings error: {e}")))?;
        ...
        for layer in self.layers.iter_mut() {
            ...
            let x_norm = layer
                .attention_norm
                .forward(&x)
                .map_err(|e| MemFuseError::Internal(format!("attn_norm error: {e}")))?;
            let attn = layer.forward_attn(&x_norm, mask.as_ref(), index_pos)?;
            let x_res = (attn + residual)
                .map_err(|e| MemFuseError::Internal(format!("residual add error: {e}")))?;
            ...
            let x_ffn_norm = layer
                .ffn_norm
                .forward(&x_res)
                .map_err(|e| MemFuseError::Internal(format!("ffn_norm error: {e}")))?;
            let x_mlp = layer.mlp_or_moe.forward(&x_ffn_norm)?;
            let x_out = (x_mlp + residual)
                .map_err(|e| MemFuseError::Internal(format!("mlp residual add error: {e}")))?;
            layer_in = x_out;
        }
        ...
        let x_norm = self
            .norm
            .forward(&layer_in)
            .map_err(|e| MemFuseError::Internal(format!("output norm error: {e}")))?;
        let x_last = x_norm
            .i((.., seq_len - 1, ..))
            .map_err(|e| MemFuseError::Internal(format!("slice last token error: {e}")))?;
