...
Fehler: **102**  |  Warnungen: **0**

## `crates/memfuse-index/src/diskann.rs`  (2 Diagnose(n))

- **ERROR [E0453]** @ `crates/memfuse-index/src/diskann.rs:1562:25`: allow(unsafe_code) incompatible with previous forbid
  - _Hinweis:_ `forbid` lint level was set on command line (`-F unsafe_code`)
- **ERROR [unsafe_code]** @ `crates/memfuse-index/src/diskann.rs:1563:28`: usage of an `unsafe` block
