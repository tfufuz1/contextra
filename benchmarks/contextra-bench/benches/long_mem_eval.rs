...
fn bench_long_mem_eval(c: &mut Criterion) {
    ensure_fd_limit(65536);
    let rt = Runtime::new().expect("Tokio runtime creation failed");
    let mut group = c.benchmark_group("long_mem_eval");
    group.sample_size(10);
    group.measurement_time(Duration::from_secs(5));

    let fixture_path = get_fixture_path("tests/fixtures/long_mem_eval_fixture.json");
    let cases =
        load_from_jsonl(&fixture_path).expect("Failed to load LongMemEval fixture dataset");
            ...
            async move {
                ...
                for _ in 0..iters {
                    ...
                    for (case_idx, case) in cases.iter().enumerate() {
                        for (turn_idx, (role, utterance, sess_idx)) in
                            case.session_history.iter().enumerate()
                        {
                    ...
                    .await
                    .expect("run_long_mem_eval failed");
...
criterion_group!(benches, bench_long_mem_eval);
