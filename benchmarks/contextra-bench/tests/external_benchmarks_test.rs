...
use contextra_bench::locomo::{load_locomo_dataset, run_locomo_eval, LocomoQuestionCategory};
...
async fn test_long_mem_eval_fixture_parsing_and_eval() {
    ...
    let mock_search = |q: &str| {
        ...
        Box::pin(async move {
            ...
            as contextra_bench::long_mem_eval::BoxFuture<
                'static,
...
async fn test_empty_and_erroring_eval_runs() {
    ...
    let empty_locomo: Vec<contextra_bench::locomo::LocomoCase> = vec![];
    ...
    let empty_lme: Vec<contextra_bench::long_mem_eval::LongMemEvalCase> = vec![];
    ...
    let mock_search_err = |_q: &str| {
        Box::pin(async {
            Err(contextra_core::ContextraError::InvalidInput(
                "Search failed".into(),
            ))
        })
            as contextra_bench::long_mem_eval::BoxFuture<
    ...
    let dummy_case = contextra_bench::long_mem_eval::LongMemEvalCase {
        question_id: "q1".into(),
        session_history: vec![],
        question: "Who?".into(),
