# Contextra — Benchmark Harness für Retrieval-Qualität (`contextra-bench`)

Dieses Verzeichnis enthält das eigenständige, reproduzierbare Benchmark-Harness `contextra-bench` zur Messung und Validierung der Retrieval-Genauigkeit von Contextra.
...
- **Wirkung**: Ohne Kontext-Präfix liefert die Volltext-Suche (BM25) für spezifische Kontext-Anfragen identische Scores für alle Chunks. Mit Kontext-Präfix injiziert Contextra das Dokumenten-Kontext-Präfix in die Indizierung, sodass BM25 das exakt gemeinte Ziel-Dokument auf Rank 1 hebt.
...
2. **Contextra-eigene Messung**: Das hiesige Benchmark-Harness `contextra-bench` misst die tatsächliche Contextra-Implementierung in isolierten, reproduzierbaren Test-Szenarien. Auf unserem synthetischen Ambiguitäts-Testkorpus erreicht Contextra aktuell:
...
3. Die gemessenen Werte dienen in erster Linie der Kontrollierbarkeit und automatisierten Regressionsvermeidung bei Code-Änderungen an der Contextra-Engine.
