...
> **Hinweis zur Migration:** IST-Stand weicht stellenweise vom normativen SOLL-Zielzustand (§A2/§4.2) ab. Wo Namen oder Ring-Zuweisungen im Code-Bestand noch dem Übergangsstadium entsprechen (z. B. `contextra-db` als Monolith statt getrennter `engine`/`cognition`/`privacy`, `contextra-index` statt `contextra-vector`, `contextra-candle` statt `contextra-infer-candle`), ist dies explizit als „In Migration" bzw. „Strangler / Legacy" gekennzeichnet. Für Details zur Ring-Zuordnung und Migrationsreihenfolge siehe `ARCHITECTURE.md` (folgt in Kürze) und §20.
...
├── schemas/
│   └── contextra.fbs                 # FlatBuffers IPC-Schema (§12)
├── crates/
│   ├── contextra/                    # Ring 4 — Shell / Facade & Composition Root (In Migration)
│   ├── contextra-adapt/              # Ring 0 — Adaptive Control & Bandit (Fertig)
│   ├── contextra-agent/              # Ring 3 — Workflow Engine, Audit & DLQ (Fertig)
│   ├── contextra-calibration/        # Ring 0 — Legacy Score Calibration (Strangler / In Migration nach adapt/rank)
│   ├── contextra-candle/             # Ring 2 — Inferenz-Candle & GGUF (In Migration nach infer-candle)
│   ├── contextra-checkpoint/         # Ring 1 — Snapshot & Time-Travel Checkpoint Registry (Fertig)
│   ├── contextra-core/               # Ring 0 — Legacy Facade & Re-exports (Strangler / In Migration)
│   ├── contextra-crypto/             # Ring 0 — Crypto Kernel, AEAD & Deletion Proofs (Fertig)
│   ├── contextra-db/                 # Ring 3 — Database Orchestrator & Collection Monolith (In Migration)
│   ├── contextra-embed/              # Ring 2 — Inferenz-ONNX Embeddings & Reranker (In Migration nach infer-onnx)
│   ├── contextra-graph/              # Ring 0 — CSR Graph, Forward-Push PPR & Hyperedges (Fertig)
│   ├── contextra-index/              # Ring 0 — HNSW, DiskANN & Quantisierung (In Migration / Vektor-Index)
│   ├── contextra-kvcache/            # Ring 1 — Tenant Isolated KV-Cache & Segment Storage (Fertig)
│   ├── contextra-mcp/                # Ring 4 — MCP Server, Stdio Protocol & Egress Guard (Fertig)
│   ├── contextra-mvcc/               # Ring 0 — SeqLog, SnapshotRegistry & TxBuffer (Fertig)
│   ├── contextra-ollama/             # Ring 2 — Ollama Inferenz Client & Context Prefixer (Fertig)
│   ├── contextra-ports/              # Ring 0 — Core Abstract Traits & Interfaces (Fertig)
│   ├── contextra-py/                 # Ring 4 — PyO3 Python Bindings (Fertig)
│   ├── contextra-router/             # Ring 3 — SLM Profile Router & Dispatcher (Fertig)
│   ├── contextra-sandbox/            # Ring 2 — WASM Execution Boundary (Fertig)
│   ├── contextra-simd/               # Ring 0 — Unsafe Island: SIMD Distance Kernels (Fertig)
│   ├── contextra-store/              # Ring 1 — LSM Storage Engine, WAL & KvKeyLocks (Fertig)
│   ├── contextra-sys/                # Ring 0 — Unsafe Island: OS System Call Wrappers (Fertig)
│   ├── contextra-testkit/            # Tooling — FaultVfs, ManualClock & In-Memory Store (Fertig)
│   ├── contextra-text/               # Ring 0 — BM25/BM25F Search & German Morphology (Fertig)
│   ├── contextra-types/              # Ring 0 — Canonical Domain Types & Error Enums (Fertig)
│   └── contextra-wire/               # Ring 0 — Unsafe Island: FlatBuffers IPC Generat & Adapters (Fertig)
├── benchmarks/
│   └── contextra-bench/              # Benchmark Harness
├── .github/workflows/
