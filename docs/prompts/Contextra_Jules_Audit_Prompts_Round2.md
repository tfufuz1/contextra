# Contextra — Google-Jules Crate-Audit-Prompts, Runde 2 (Vertiefung & Dekomposition)
...
|---|---|---|
| Fehlendes `fsync` im WAL → Silent Data Loss | `contextra-store`-Audit fand **5 kritische Bugs** in Compaction/MemTable/SSTable/Snapshot-Pfad (bereits behoben) — fsync selbst nicht explizit widerlegt | **Erneut prüfen**, gezielt fsync-Pfad isoliert verifizieren (Runde 1 hat es nicht explizit als eigenen Prüfpunkt geführt) |
...
| 2PC-Deadlocks zwischen Sub-Engines | `contextra-db`-Audit bestätigt Lock-Hierarchie-Konformität (Tabellen-Audit) und 10.000+ Op Stresstest ohne Deadlock — **aber**: Zettelkasten-Displacement-Zyklen (`Supersedes`-Relation) wurden nicht geprüft | **Neu, gezielt prüfen** |
...
| Router (`contextra-router`) verletzt Layer-DAG durch Import aus `contextra-mcp` | **Im aktuellen Code-Stand nicht nachweisbar** (kein `contextra_mcp`-Import in `contextra-router/src/`) — vermutlich bereits behoben oder Fehleinschätzung | **Nur Regressionscheck**, kein Schwerpunkt |
...
# A. `contextra-db` — Dekomposition in 4 Sub-Prompts

## A.1 `contextra-db` / Zettelkasten-Displacement & Relation-Zyklen

```
ROLLE
Senior Rust Datenbank-Architekt, spezialisiert auf Graph-Constraint-Validierung in relationalen/
dokumentbasierten Systemen. Du vertiefst den bestehenden Audit von `contextra-db` (siehe
docs/audits/AUDIT_contextra-db.md, Abschnitt 3-4, Status: Lock-Hierarchie & Fusion bereits BESTANDEN)
...
die Post-RRF-Filterung oder die Graph-Traversierung (`collection/relate.rs`, `contextra-graph`-Integration)
...
AUFGABENUMFANG
1. Analysiere `crates/contextra-db/src/collection/relate.rs` vollständig: identifiziere JEDE Stelle, an
...
REPORT: docs/audits/round2/AUDIT_contextra-db_relation-cycles.md
...
## A.2 `contextra-db` / Snapshot-Isolation bei Vektor- und Graph-Signalen (Read-Uncommitted-Fenster)
...
MISSION
Der Runde-1-Audit hat MVCC/Snapshot-Isolation für den LSM-Pfad (`contextra-store`) bestätigt und
Lock-Hierarchie-Konformität in `contextra-db` nachgewiesen — ABER: keiner der bisherigen Reports hat
...
AUFGABENUMFANG
1. Analysiere den Suchpfad in `crates/contextra-db/src/collection/search.rs` und `fusion.rs`: für JEDES
   der 4 Signale, identifiziere exakt, GEGEN WELCHEN Snapshot/Zustand die jeweilige Sub-Engine
   (`contextra-index::hnsw`, `contextra-text::bm25`, `contextra-graph::csr`, Metadaten-Filter) tatsächlich
   liest — wird ein `SnapshotRegistry`-Handle (aus `contextra-core`) an alle vier durchgereicht, oder
...
REPORT: docs/audits/round2/AUDIT_contextra-db_cross-signal-isolation.md
...
## A.3 `contextra-db` / 2PC-Fehlerpfad-Erschöpfung & Kompensations-Rollback unter Fault-Injection
...
bestätigt (`test_4_index_atomic_rollback_on_vector_failure`, siehe AUDIT_contextra-db.md Abschnitt 4).
...
AUFGABENUMFANG
1. Analysiere `crates/contextra-db/src/transaction.rs` und identifiziere die exakte Reihenfolge, in der
...
REPORT: docs/audits/round2/AUDIT_contextra-db_2pc-fault-injection.md
...
## A.4 `contextra-db` / RRF-Performance unter Realistischer Ergebnismenge (Eager-Materialization-Verdacht)
...
AUFGABENUMFANG
1. Analysiere `crates/contextra-db/src/fusion.rs`: bestimmt die Implementierung, wie viele Elemente
...
REPORT: docs/audits/round2/AUDIT_contextra-db_rrf-scaling.md
...
# B. `contextra-index` — Dekomposition in 4 Sub-Prompts

## B.1 `contextra-index` / CPU-Feature-Detection-Vollständigkeit (Illegal-Instruction-Risiko)
...
MISSION
Runde 1 (`AUDIT_contextra-index.md`) hat drei kritische Bugs behoben (DiskANN-Recall, zwei mmap-Slice-
...
AUFGABENUMFANG
1. Durchsuche `crates/contextra-index/src/distance.rs` vollständig nach JEDEM Vorkommen eines
...
REPORT: docs/audits/round2/AUDIT_contextra-index_cpu-feature-detection.md
...
## B.2 `contextra-index` / mmap-TOCTOU-Race unter aktivem Dateizugriff (DiskANN)
...
AUFGABENUMFANG
1. Analysiere den Lebenszyklus des Mmap-Handles in `crates/contextra-index/src/diskann.rs` und
...
REPORT: docs/audits/round2/AUDIT_contextra-index_mmap-toctou.md
...
## B.3 `contextra-index` / Kompositazerlegungs-Root-Cause via Morphologie-Grenzfall (Cross-Crate: contextra-text-Bezug für Recall-Wirkung)

```
ROLLE
Senior Rust Search-Quality-Ingenieur — dieser Sub-Prompt ist bewusst hier bei contextra-index verortet
(nicht contextra-text), weil untersucht wird, wie ein Text-seitiger Recall-Verlust sich auf die
4-Signal-Fusion-Gesamtqualität auswirkt, für die contextra-index das dominante Signal liefert. Für die
reine morphologische Root-Cause-Analyse siehe den korrespondierenden Sub-Prompt C.3 unter contextra-text.

MISSION
Der Runde-1-Text-Audit (AUDIT_contextra-text.md) fand: 41/45 deutsche Komposita korrekt zerlegt, aber 3
...
2. Führe End-to-End-Hybridsuchen über `contextra-db` mit Queries durch, die exakt nach Teilbegriffen
...
REPORT: docs/audits/round2/AUDIT_contextra-index_compound-split-recall-impact.md
...
4) Neubewertung des Schweregrads, 5) Empfehlung (Priorität für Fix in contextra-text hoch/mittel/niedrig
...
## B.4 `contextra-index` / HNSW Fine-Grained-Locking-Verdacht unter Schreiblast (Insert-Skalierung)
...
AUFGABENUMFANG
1. Analysiere `crates/contextra-index/src/hnsw.rs`: identifiziere die exakte Lock-Granularität beim
...
REPORT: docs/audits/round2/AUDIT_contextra-index_hnsw-lock-granularity.md
...
# C. `contextra-store` — Dekomposition in 4 Sub-Prompts

## C.1 `contextra-store` / fsync-Durability-Isolationstest (Silent-Data-Loss-Hypothese)
...
AUDIT_contextra-store.md, Executive Summary) — ein beeindruckendes Ergebnis. ABER: keiner der
...
AUFGABENUMFANG
1. Durchsuche `crates/contextra-store/src/wal.rs` nach JEDEM Aufruf von `sync_all()`, `sync_data()`, oder
...
REPORT: docs/audits/round2/AUDIT_contextra-store_fsync-durability.md
...
## C.2 `contextra-store` / HMAC-Truncation-Attack & Metadaten-Bindung im WAL (Cross-Crate mit contextra-crypto)
...
MISSION
Der bereits durchgeführte `contextra-crypto`-Audit (Runde 1, siehe docs/audits/AUDIT_contextra-crypto.md)
...
AUFGABENUMFANG
1. Analysiere `crates/contextra-crypto/src/anti_tamper.rs` UND die Aufrufstelle in
   `crates/contextra-store/src/wal.rs` GEMEINSAM: was genau fließt exakt in die HMAC-Eingabe ein? Nur der
...
2. Konstruiere folgenden Angriffstest: schreibe eine gültige WAL-Sequenz aus mindestens 5 Blöcken.
   ...
   d) Extrahiere Block 3 aus dieser WAL-Datei und füge ihn in eine ANDERE, aber strukturell ähnliche
      ...
      werden (Bezug zur Key-Separation-pro-Datei-Invariante aus dem contextra-crypto-Audit).
...
REPORT: docs/audits/round2/AUDIT_contextra-store_wal-hmac-binding.md
...
## C.3 `contextra-store` / Write-Amplification & Bloom-Filter-Vorhandensein unter realistischem Workload
...
AUFGABENUMFANG
1. Durchsuche `crates/contextra-store/src/sstable.rs` und `compaction.rs` nach Bloom-Filter-Strukturen
...
REPORT: docs/audits/round2/AUDIT_contextra-store_write-read-amplification.md
...
## C.4 `contextra-store` / Async-I/O-Blocking-Verifikation (Executor-Starvation-Risiko)
...
(inklusive des MCP-Servers `contextra-mcp`, der auf demselben Runtime-Kontext läuft), was dazu führt, dass
das System unter I/O-Last nicht mehr auf externe Anfragen reagiert. Der FILE-CONTEXT-Kommentar in
`crates/contextra-store/src/lib.rs` behauptet explizit die Einhaltung dieser Regel ("tokio::fs für
...
AUFGABENUMFANG
1. Durchsuche das GESAMTE `contextra-store`-Crate nach JEDEM Vorkommen von `std::fs::File`-Operationen
...
3. Führe einen empirischen Beweistest durch (unabhängig vom Code-Review, da Code-Review menschliche/KI-
   ...
   über die öffentliche `contextra-store`-API. WÄHREND diese läuft, starte auf DEMSELBEN Tokio-Runtime-
...
REPORT: docs/audits/round2/AUDIT_contextra-store_async-blocking.md
...
# D. `contextra-graph` — Dekomposition in 3 Sub-Prompts

## D.1 `contextra-graph` / CSR-Insert-Komplexität — Delta-Graph-Kosten-Verifikation
...
AUFGABENUMFANG
1. Analysiere `crates/contextra-graph/src/csr.rs`: identifiziere den exakten Trigger-Mechanismus für eine
...
REPORT: docs/audits/round2/AUDIT_contextra-graph_csr-insert-complexity.md
...
## D.2 `contextra-graph` / PPR Sink-Hole & Teleportations-Vektor-Korrektheit unter Dangling-Nodes
...
AUDIT_contextra-graph.md), aber laut der Gemini-Analyse ist der GEFÄHRLICHSTE PPR-Fehlerfall spezifisch
...
AUFGABENUMFANG
1. Prüfe zuerst den Runde-1-Report (AUDIT_contextra-graph.md, PPR-Abschnitt): enthielt der dortige
...
5. Falls in irgendeinem der Fälle Score-Erhalt verletzt wird oder ein "Score-Kollaps"-Muster
   ...
   `crates/contextra-graph/src/ppr.rs` als Root Cause.

REPORT: docs/audits/round2/AUDIT_contextra-graph_ppr-dangling-nodes.md
...
## D.3 `contextra-graph` / BFS-Explosion an Hub-Knoten (Branching-Limit-Verifikation)
...
AUFGABENUMFANG
1. Analysiere die BFS-Implementierung in `crates/contextra-graph/src/csr.rs`: wird `MAX_SEARCH_K` als
...
REPORT: docs/audits/round2/AUDIT_contextra-graph_hub-node-bfs-explosion.md
...
# E. `contextra-text` — Dekomposition in 3 Sub-Prompts

## E.1 `contextra-text` / Root-Cause der Kompositazerlegungs-Fehlschläge & Rekursions-/Backtracking-DoS-Analyse
...
MISSION
Runde 1 (AUDIT_contextra-text.md) fand: 41/45 Komposita korrekt gesplittet, aber die 3 längsten/
...
AUFGABENUMFANG
1. Lies `crates/contextra-text/src/morphology.rs` (`GermanCompoundSplitter`) vollständig und rekonstruiere
...
REPORT: docs/audits/round2/AUDIT_contextra-text_compound-split-root-cause.md
...
## E.2 `contextra-text` / Unicode-Grapheme-Grenzen-Panic-Erschöpfung (Byte- vs. Char-Slicing)
...
AUFGABENUMFANG
1. Durchsuche das GESAMTE `contextra-text`-Crate (alle 5 Dateien: bm25.rs, inverted.rs, morphology.rs,
...
REPORT: docs/audits/round2/AUDIT_contextra-text_unicode-slicing-panics.md
...
## E.3 `contextra-text` / Allokations-Overhead-Audit (Zero-Copy-Verstoß-Hypothese)
...
AUFGABENUMFANG
1. Durchsuche `crates/contextra-text/src/{tokenizer,morphology,inverted,bm25}.rs` nach JEDEM Vorkommen von
...
REPORT: docs/audits/round2/AUDIT_contextra-text_allocation-overhead.md
...
```
F.1 contextra-core — TxId-Allocation-Base-Ranges unter absichtlicher Erschöpfungssimulation
...
Report: docs/audits/round2/AUDIT_contextra-core_txid-range-boundary.md

F.2 contextra-router — Regressionscheck der Layer-DAG-Konformität + Determinismus bei SlmProfile-Änderung
zur Laufzeit
Ziel: Bestätige per Grep, dass contextra-router weiterhin KEINE contextra-mcp-Typen importiert (Regression
...
Report: docs/audits/round2/AUDIT_contextra-router_dag-and-hotreload.md

F.3 contextra-ollama / contextra-tauri — Erweiterte Prompt-Injection-Denylist-Härtungsprüfung
...
Report: docs/audits/round2/AUDIT_contextra-ollama-tauri_prompt-injection-hardening.md

F.4 contextra-checkpoint — Nebenläufiger Time-Travel unter gleichzeitigem Rollback zweier Agenten-Sessions
...
der anderen).
Report: docs/audits/round2/AUDIT_contextra-checkpoint_concurrent-sessions.md

F.5 contextra-agent — Token-Budget-Race zwischen parallelen Schritten derselben Workflow-Instanz
...
Report: docs/audits/round2/AUDIT_contextra-agent_budget-race.md

F.6 contextra-mcp — Slowloris-artiger Verbindungsaufbau über stdio (partielle Requests über sehr lange Zeit)
...
Report: docs/audits/round2/AUDIT_contextra-mcp_slowloris.md

F.7 contextra-embed / contextra-db (Cross-Crate) — Reranker-Score-Manipulation durch adversariale Chunk-Inhalte
...
Report: docs/audits/round2/AUDIT_contextra-embed-db_reranker-adversarial.md

F.8 contextra-py — GIL-Freigabe-Regressionstest unter Python-3.12/3.13-Sub-Interpreter-Modus (falls
zutreffend)
...
Report: docs/audits/round2/AUDIT_contextra-py_subinterpreter.md
```
