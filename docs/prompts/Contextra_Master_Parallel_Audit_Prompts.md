# Contextra — Master Parallel Audit & Remediation Prompts for Google-Jules
...
1. [Prompt 1: Concurrency, State, Race Conditions & MVCC (`contextra-core`, `contextra-db`)](#prompt-1-concurrency-state-race-conditions--mvcc)
2. [Prompt 2: Storage Engine, LSM-Trees & Transaktionen (`contextra-store`)](#prompt-2-storage-engine-lsm-trees--transaktionen)
3. [Prompt 3: KI-Inferenz, Graph & Vektorsuche (`contextra-db`, `contextra-router`, `contextra-embed`)](#prompt-3-ki-inferenz-graph--vektorsuche)
4. [Prompt 4: Low-Level, Memory, SIMD & OS-Interaktion (`contextra-index`, `contextra-store`)](#prompt-4-low-level-memory-simd--os-interaktion)
5. [Prompt 5: Security, Kryptografie & Privacy (`contextra-crypto`, `contextra-ollama`)](#prompt-5-security-kryptografie--privacy)
6. [Prompt 6: Architektur, DAG-Constraints & Code Smells (`contextra-core`, `contextra-db`, `contextra-agent`)](#prompt-6-architektur-dag-constraints--code-smells)
7. [Prompt 7: FFI, PyO3 & Cross-Language Interoperability (`contextra-py`)](#prompt-7-ffi-pyo3--cross-language-interoperability)
8. [Prompt 8: Frontend, IPC & Ingestion-Parser-Robustheit (`contextra-tauri`)](#prompt-8-frontend-ipc--ingestion-parser-robustheit)
9. [Prompt 9: Suchalgorithmen, RRF & Indizes (`contextra-text`, `contextra-index`, `contextra-graph`)](#prompt-9-suchalgorithmen-rrf--indizes)
10. [Prompt 10: Advanced Concurrency, MVCC & Lock-Management (`contextra-db`, `contextra-checkpoint`)](#prompt-10-advanced-concurrency-mvcc--lock-management)
11. [Prompt 11: Kryptografie, Anti-Tamper & Durability (`contextra-crypto`, `contextra-store`)](#prompt-11-kryptografie-anti-tamper--durability)
12. [Prompt 12: Testing, CI/CD, Anti-Leakage & Governance (`contextra-agent`, `contextra-checkpoint`, `xtask`)](#prompt-12-testing-cicd-anti-leakage--governance)
...
- `crates/contextra-core/src/snapshot.rs`
- `crates/contextra-db/src/collection/tx.rs`
...
1. Analysiere `tx_buffer.rs` und `snapshot.rs` in `contextra-core`. Stelle sicher, dass `TxBuffer` Bounded-Backpressure (Obergrenze für Staged Transactions) erzwingt und `ContextraError::InvalidInput` oder `TransactionAborted` bei Kapazitätsüberlauf zurückgibt.
...
3. Überprüfe die Lock-Hierarchie in `contextra-db/src/transaction.rs`:
...
5. Führe Verifikationstests durch:
   `cargo test -p contextra-core --lib tx_buffer`
   `cargo test -p contextra-db --lib transaction`
...
EXAKTE IMPLEMENTIERUNGSSCHRITTE & AUFGABEN
1. Analysiere `wal.rs` in `contextra-store`: Verifiziere das Append-Format, CRC32/BLAKE3-Checksum-Validierung pro Record und die strikte WAL-First-Regel (WAL commit + `fsync` vor MemTable Write).
2. Verifiziere, dass `fsync` (`sync_all()`) bei jedem Commit aufgerufen und Fehler ordnungsgemäß via `Result<T, ContextraError>` propagiert werden (kein `let _ = sync_all()`).
...
5. Führe Verifikationstests durch:
   `cargo test -p contextra-store --all-features`
...
FOKUSSIERTE CRATES & DATEIEN
- `crates/contextra-router/src/router.rs`
- `crates/contextra-db/src/fusion.rs`
- `crates/contextra-db/src/collection/query_builder.rs`
- `crates/contextra-embed/src/reranker.rs`
- `crates/contextra-ollama/src/importance.rs`
...
EXAKTE IMPLEMENTIERUNGSSCHRITTE & AUFGABEN
1. Überprüfe `query_builder.rs` und `fusion.rs` in `contextra-db`: Stelle sicher, dass Metadaten-Filter *vor* Top-K-Truncation angewendet werden (Filter-before-Truncation Invariante).
2. Analysiere `router.rs` in `contextra-router`: Verifiziere, dass Conformal Calibration erst nach `CALIBRATION_WARMUP_WINDOW >= 30` Samples aktiviert wird und `min_relevance_score.is_finite()` erzwingt.
...
FOKUSSIERTE CRATES & DATEIEN
- `crates/contextra-index/src/distance.rs`
- `crates/contextra-index/src/diskann.rs`
- `crates/contextra-index/src/persistence.rs`
- `crates/contextra-store/src/mmap.rs`
...
EXAKTE IMPLEMENTIERUNGSSCHRITTE & AUFGABEN
1. Analysiere `distance.rs` in `contextra-index`: Überprüfe alle SIMD-Intrinsics (AVX-512, AVX2, NEON, Skalar) auf strikte Vektor-Längen-Validierung.
2. Stelle sicher, dass `cosine_distance`, `euclidean_distance` und `dot_product_distance` bei Vektor-Längen-Fehlanpassung `ContextraError::EmbeddingDimensionMismatch` zurückgeben und niemals paniken.
...
5. Führe Verifikationstests aus:
   `cargo test -p contextra-index --all-features`
...
- `crates/contextra-ollama/src/client.rs`
...
EXAKTE IMPLEMENTIERUNGSSCHRITTE & AUFGABEN
1. Analysiere `crypto.rs` und `anti_tamper.rs` in `contextra-crypto`: Verifiziere, dass alle Vergleiche von Keys, Tags und HMACs die Trait-Methode `subtle::ConstantTimeEq` verwenden.
...
3. Analysiere `build_rag_prompt` und `xml_escape` in `contextra-ollama/src/client.rs`: Stelle sicher, dass XML-Sonderzeichen (`<`, `>`, `&`, `"`, `'`) konsequent maskiert werden.
4. Prüfe, dass secret Keys und Plaintext-Buffers nach Nutzung via `zeroize` genullt werden.
5. Führe Verifikationstests aus:
   `cargo test -p contextra-crypto`
   `cargo test -p contextra-ollama`
...
FOKUSSIERTE CRATES & DATEIEN
- `crates/contextra-core/src/types.rs`
- `crates/contextra-core/src/error.rs`
- `crates/contextra-core/src/error_dto.rs`
- `crates/contextra-graph/src/csr.rs`
- `crates/contextra-db/src/lib.rs`
...
2. Analysiere `Edge` in `contextra-core` und `contextra-graph`: Verifiziere die strikte Trennung von Transaction Time (`tx_valid_from`, `tx_valid_to`: `Option<TxId>`) und Business Time (`business_valid_from`, `business_valid_to`: `Option<i64>`).
...
4. Überprüfe `ContextraError`: Verifiziere die Zero-Panic-Doktrin (kein `.unwrap()` oder `.expect()` außerhalb von `#[cfg(test)]`).
5. Führe Verifikationstests aus:
   `cargo test -p contextra-core`
   `cargo test -p contextra-graph`
...
KOMPETENZEN
- Experte für PyO3, Python Sub-Interpreter, Zero-Copy Buffers, GIL Deadlock Prevention und Safe Exception Mapping (`ContextraError` -> `PyErr`).

FOKUSSIERTE CRATES & DATEIEN
- `crates/contextra-py/src/lib.rs`
- `crates/contextra-core/src/error_dto.rs`
...
EXAKTE IMPLEMENTIERUNGSSCHRITTE & AUFGABEN
1. Analysiere `crates/contextra-py/src/lib.rs`: Stelle sicher, dass jede öffentlich exponierte PyO3-Funktion mit `pyo3::panic::catch_unwind` oder sauberer `Result<T, PyErr>`-Rückgabe geschützt ist.
...
3. Verifiziere, dass `OnceLock`-geteilte Tokio-Runtimes in `contextra-py` gegen Re-Initialization in Sub-Interpretern abgesichert sind.
...
FOKUSSIERTE CRATES & DATEIEN
- `crates/contextra-tauri/src/commands/mod.rs`
- `crates/contextra-tauri/src/ingestion/pdf.rs`
- `crates/contextra-tauri/src/ingestion/docx.rs`
- `crates/contextra-tauri/src/ingestion/email.rs`
...
EXAKTE IMPLEMENTIERUNGSSCHRITTE & AUFGABEN
1. Überprüfe `validate_path_within_base` in `crates/contextra-tauri/src/commands/mod.rs`: Stelle sicher, dass Pfad-Traversal absolut verhindert wird.
...
3. Verifiziere, dass alle `#[tauri::command]` Funktionen Fehler als `Result<T, ContextraErrorDto>` zurückgeben und nicht als `Result<T, String>`.
4. Führe Verifikationstests aus:
   `cargo test -p contextra-tauri`
...
FOKUSSIERTE CRATES & DATEIEN
- `crates/contextra-text/src/bm25.rs`
...
3. Analysiere `csr.rs` in `contextra-graph`:
...
4. Führe Verifikationstests aus:
   `cargo test -p contextra-text`
   `cargo test -p contextra-index`
   `cargo test -p contextra-graph`
   `cargo test -p contextra-db`
...
FOKUSSIERTE CRATES & DATEIEN
- `crates/contextra-db/src/collection/search.rs`
- `crates/contextra-db/src/fusion.rs`
...
EXAKTE IMPLEMENTIERUNGSSCHRITTE & AUFGABEN
1. Überprüfe `search.rs` und `fusion.rs` in `contextra-db`: Stelle sicher, dass während einer 4-Signal-Suche ein einheitliches `SnapshotRegistry`-Handle an alle Sub-Engines durchgereicht wird.
2. Überprüfe `PersistentCheckpointStore` in `contextra-checkpoint`: Verifiziere Session-Isolation bei parallelem Time-Travel.
...
4. Führe Verifikationstests aus:
   `cargo test -p contextra-db`
   `cargo test -p contextra-checkpoint`
...
4. Führe Verifikationstests aus:
   `cargo test --workspace --exclude contextra-tauri`
