...
SIEVE verzichtet vollständig auf Listen-Neuordnung bei Lesetreffern. Ein Cache-Hit reduziert sich auf das Setzen
eines einzigen atomaren `visited`-Bits (`Ordering::Relaxed`), ohne jede Mutation der Listenstruktur. Eviction
erfolgt über einen umlaufenden Zeiger („Hand"): gesetztes `visited`-Bit → begnadigt (Bit gelöscht, verbleibt im
Cache), gelöschtes Bit → verdrängt.
...
**Sharding:** Beide Backends werden über ein `ShardedBlockCache<K, V, B: BlockCacheBackend<K,V>>` mit
...
Kapazität byte-basiert statt eintragsbasiert führen (Eviction anhand der tatsächlichen Bytegröße).
...
Stattdessen wird eine kohärente, erstklassige Rust-Struktur mit Zero-Copy-Deserialisierung via FlatBuffers
spezifiziert.
