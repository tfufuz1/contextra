# Contextra Spezifikation v4 — Index und Kapitelübersicht

Dieses Verzeichnis `docs/spec/` enthält die vollständige, verbindliche Gesamtspezifikation **Contextra v4 Master**, zerlegt in modulare Kapiteldateien. Jede Kapiteldatei ist auf maximal 25 KB begrenzt und trägt am Dateianfang einen YAML-Frontmatter-Header.
...
| `07-retrieval-pipeline.md` | ## 7. Retrieval-Pipeline: 4-Signal-Fusion und ihre Algorithmen | Ring 0/1/3 (`contextra-text`, `contextra-graph`, `contextra-index`, `contextra-rank`) |
| `08-contextual-bandit-routing.md` | ## 8. Contextual-Bandit-Routing | Ring 0/3 (`contextra-adapt`, `contextra-router`) |
| `09-inferenz-kv-cache-ipc.md` | ## 9. Inferenz, KV-Cache v2 und Zero-Copy-IPC | Ring 0/1/3 (`contextra-kvcache`, `contextra-wire`, `contextra-candle`) |
...
| `90-anhang-b-begruendungen-b.md` | # Anhang B — Begründungen, Ist-Zustand, Literatur (Teil B: §B.5.3–§B.7) | Ring 0/1 (`contextra-store`, `contextra-index`) |
