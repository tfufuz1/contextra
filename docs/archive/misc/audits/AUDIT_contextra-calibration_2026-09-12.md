...
**Crate:** `contextra-calibration` (Layer 3 — Kalibrierung, Uncertainty & Latenz-Regelung)  
**Auditor Persona:** Senior Rust Performance & Systems Engineer (Contextra)  
...
`contextra-calibration` umfasst insgesamt **1.773 Zeilen** über 4 Quelldateien unter `src/` und 2 Integrationstest-Dateien unter `tests/`:
...
Ein workspace-weiter Scan (`grep -rn "PlattScaler"`) widerlegt die vorherige Vermutung, `PlattScaler` habe keine produktiven Aufrufer. `PlattScaler` ist in `crates/contextra-embed/src/reranker.rs` (`CrossEncoderReranker`) vollständig im produktiven Reranking-Pfad integriert:

- **Alias & Type Definition (`reranker.rs:21`):** `pub use contextra_calibration::PlattScaler as PlattScaledSigmoid;`
...
1. **Produktiv (15 Aufrufe):** `crates/contextra-embed/src/reranker.rs:17, 21, 47, 63, 73, 159, 433, 453, 469, 481, 487, 516, 947, 1017, 1052, 1059`.
2. **Tests & Modultests (8 Aufrufe):** `crates/contextra-calibration/tests/calibration_deep_tests.rs:6, 149, 162, 170, 175, 180, 186, 275`.

---

### 2.2 Status von `replicator.rs` (`ReplicatorState`) — **OBSOLET / IN `contextra-calibration` GELÖSCHT**
Ein Scan nach `replicator.rs` im Crate `contextra-calibration` zeigt:
- Die Datei `crates/contextra-calibration/src/replicator.rs` **existiert nicht mehr**.
- In `Cargo.toml` existiert zwar noch das leere Feature-Flag `replicator-dynamics-weights = []`, das Modul selbst wurde jedoch aus `contextra-calibration` entfernt.
- **Folgefehler in `contextra-db`:** In `crates/contextra-db` existieren veraltete Import-Versuche (`use contextra_calibration::ReplicatorState;` in `query_builder.rs`, `maintenance_scheduler.rs`, `cross_domain_chaos_matrix_test.rs`), die bei Compilierung von `contextra-db` mit `E0425: cannot find type ReplicatorState in crate contextra_calibration` fehlschlagen.
...
1. **Latenzmessung (`crates/contextra-db/src/collection/query_builder.rs:453`):**
...
4. **Auswirkung im Search Engine Pfad (`crates/contextra-db/src/collection/search.rs:906`):**
...
- `contextra-calibration` hat 1.773 Zeilen (Quellcode + Tests) und ist damit sehr kompakt.
...
- `GaspValidator` (in `contextra-candle`) nutzt `IsotonicCalibrator` und löst bei Fingerprint-Drift die Invalidation verlässlich aus.
...
1. **Unit- & Integrationstests (`cargo test -p contextra-calibration --all-features -- --include-ignored`):**
   ```text
   ...
   ```
2. **Clippy Linter (`cargo clippy -p contextra-calibration --all-features -- -D warnings`):**
   ```text
...
1. **Fix APM-2 in `HybridQueryBuilder::execute()` (`crates/contextra-db/src/collection/query_builder.rs`):**
   ...
   ```
2. **Bereinigung veralteter `ReplicatorState`-Referenzen in `contextra-db`:**
   Entweder das Modul `replicator.rs` in `contextra-calibration` wiedereinführen oder veraltete Importe und Felder in `contextra-db` bereinigen, um den Compilierfehler `E0425` zu beheben.
