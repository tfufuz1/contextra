...
Im Rahmen der systematischen Auditierung des Layer-8 Crates `contextra-mcp` wurden der 6-Punkte-Prüfkatalog, 5 aktive Prompt-Injection-Umgehungsversuche, ein Permission-Bypass-Test sowie eine lückenlose Input-Validierungs-Inventur durchgeführt.
...
2. **Execution**: Aufruf von Schreib-Tools (`contextra_insert`, `contextra_delete`, `contextra_consolidate`) sowie unklassifizierten Code-Tools (`unknown_code_tool`).
3. **Ergebnis**:
   - `contextra_insert` -> **VERWEIGERT** (`"Sandbox: DB-Schreibzugriff gesperrt für 'contextra_insert'"`)
   - `contextra_consolidate` -> **VERWEIGERT** (`"Sandbox: DB-Schreibzugriff gesperrt für 'contextra_consolidate'"`)
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `contextra_search` | `query` | String | Non-empty, max 64 KB | `s.trim().is_empty()` & `s.len() > MAX_SEARCH_QUERY_BYTES` (64KB) | **Keine** (Vollständig) | OK |
| `contextra_search` | `collection` | String | Valid Name (no `\0`, `:`, `/`, len<=256) | `validate_collection_name(s)` | **Keine** (Vollständig) | OK |
| `contextra_search` | `k` / `limit` | Integer | Positive Ganzzahl $\ge 1$, capped at `MAX_SEARCH_K` (10.000) | `n.as_u64()` check, `.min(MAX_SEARCH_K)` | **Keine** (Vollständig) | OK |
| `contextra_insert` | `id` | String | Non-empty, max 256 Chars, valid `DocId` | `s.trim().is_empty()`, `s.len() > 256`, `DocId::from_key()` | **Keine** (Vollständig) | OK |
| `contextra_insert` | `text` | String | Optional, non-empty, max 10 MB | `s.trim().is_empty()`, `s.len() > 10MB` | **Keine** (Vollständig) | OK |
| `contextra_insert` | `vector` | Array | Non-empty, finite f32 floats | `arr.is_empty()`, `f.is_nan()`, `f.is_infinite()` | **Keine** (Vollständig) | OK |
| `contextra_insert` | `metadata` | Object | Valid JSON Object | `v.as_object()` | **Unbounded Metadata Payload Size** (nur beschränkt durch 4MB RPC Line) | **Niedrig** |
| `contextra_get` | `id` | String | Non-empty, max 256 Chars | `s.trim().is_empty()` | **LÜCKE**: Keine explizite Längenbegrenzung `id.len() <= 256` vor `col.get()` | **Niedrig** |
| `contextra_get` | `collection` | String | Valid Name | `validate_collection_name(s)` | **Keine** (Vollständig) | OK |
| `contextra_collections` | - | - | keine Params | - | **Keine** | OK |
| `contextra_consolidate` | `collection` | String | Valid Name | `validate_collection_name(s)`, Type-Check string | **Keine** (Vollständig) | OK |
...
- `crates/contextra-mcp/src/lib.rs` erzwingt `#![forbid(unsafe_code)]` in Zeile 1.
...
- Parameter-Validierung aller 5 MCP-Tools ist robust. Kleiner Befund bei `contextra_get` (fehlende ID-Längenbegrenzung auf 256 Bytes).
- **Ergebnis**: **PASSED**
...
| :--- | :--- | :--- |
| **P1 (Hoch)** | **Homoglyph-Faltung im Guard**: Ergänzung einer Skeleton/ASCII-Confusable Normalisierung (z.B. via `deunicode` oder Latin Skeleton Mapping) vor der Mustersuche in `normalize_text`. | `crates/contextra-mcp/src/prompt_injection.rs` |
| **P2 (Mittel)** | **URL-Decoding vor Rekursion**: `decode_base64()` Output bzw. Dekodierter String sollte vor der rekursiven Mustersuche zusätzlich URL-dekodiert werden. | `crates/contextra-mcp/src/prompt_injection.rs` |
| **P2 (Mittel)** | **Erweiterte Zero-Width Filterung**: `is_zero_width()` um Unicode General Category `Format` (`Cf`) erweitern (z.B. `\u{061C}`, `\u{E0001}`..=`\u{E007F}`). | `crates/contextra-mcp/src/prompt_injection.rs` |
| **P3 (Niedrig)** | **`contextra_get` ID-Längenbegrenzung**: Explizite Prüfung `id.len() <= 256` in `contextra_get` analog zu `contextra_insert` hinzufügen. | `crates/contextra-mcp/src/lib.rs` |
...
cargo clippy -p contextra-mcp --all-features -- -D warnings
