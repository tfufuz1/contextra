# Audit-Bericht: `contextra-tauri` ("Contextra Brain" Desktop Application Shell)

**Datum**: 2026-08-30 (Aktualisiert: 2026-09-02)
**Auditor**: Senior Rust Desktop Application Architect & Security Specialist
**Ziel-Crate**: `crates/contextra-tauri` (`contextra_tauri_lib` / `contextra-brain`)

---

## 1. Executive Summary

`contextra-tauri` bildet die Schnittstelle zwischen der Tauri v2 Frontend-Oberfläche und dem Kern-Memory-Engine `contextra-db`. Da über diese Schicht Dokumente (PDF, DOCX, E-Mail/EML, Markdown, TXT) aus potenziell nicht vertrauenswürdigen Quellen vom Benutzer importiert werden, wurde `contextra-tauri` einer Tiefenprüfung auf Parser-Robustheit, IPC-Sicherheit, Thread-Safety und IPC-Fehler-Handhabung unterzogen.
...
```rust
pub struct AppState {
    pub db: RwLock<Option<Arc<Contextra>>>,
...
1. **Verwendung von `parking_lot::RwLock`**: `db` und `db_path` sind durch `parking_lot::RwLock` geschützt. Bei Lesezugriffen in Tauri-Commands (`list_collections`, `search`, `ingest`, `chat_with_rag`) wird `state.db.read()` nur für den Bruchteil einer Mikrosekunde gehalten, um den `Arc<Contextra>` Pointer zu klonen (`ok_or(...)?.cloned()`). Locks werden nicht über `await`-Punkte gehalten.
...
3. **Chunking**: Nutzt `contextra_db::chunker::MarkdownChunker` für semantisches Splitting.
...
|---|---|---|---|---|---|
| `open_database` | `collections.rs` | Erstellt Pfad falls nicht existent, canonicalize | N/A | `ContextraErrorDto` | **PASS** |
| `list_collections` | `collections.rs` | Prüft ob DB offen | N/A | `ContextraErrorDto` | **PASS** |
| `create_collection` | `collections.rs` | `validate_collection_name` (Länge <=256, keine `__` Präfixe, Alpha-Numeric) | N/A | `ContextraErrorDto` | **PASS** |
| `drop_collection` | `collections.rs` | `validate_collection_name` | N/A | `ContextraErrorDto` | **PASS** |
| `ingest_file` | `ingest.rs` | Collection-Name Validierung, Datei-Existenz-Prüfung | `validate_path_within_base` gegen DB-Pfad | `ContextraErrorDto` | **PASS** |
| `ingest_folder` | `ingest.rs` | Directory-Prüfung | `validate_path_within_base` | `ContextraErrorDto` | **PASS** |
| `hybrid_search` | `search.rs` | Query-Längen-Limit (`MAX_QUERY_LEN = 64KB`) | N/A | `ContextraErrorDto` | **PASS** |
| `chat_with_rag` | `chat.rs` | Query-Längen-Limit (`MAX_QUERY_LEN = 64KB`), Event-Emission | N/A | `ContextraErrorDto` | **PASS** |
| `run_regex_transform` | `transform.rs` | Pattern-Compile-Validierung, Input-Limit (1MB normal / 64KB komplex) | N/A | `ContextraErrorDto` | **PASS** |
| `run_bulk_regex_transform` | `transform.rs` | Semaphore-Permit Acquisition pro Element | N/A | `ContextraErrorDto` | **PASS** |
...
### Architektur-Vergleich: `OllamaBridge` vs. `contextra-ollama::OllamaClient`
- `OllamaClient` in `contextra-ollama` ist die Kern-HTTP-Client-Implementierung (Layer 1).
- `OllamaBridge` in `contextra-tauri` (`src/ollama.rs`) ist eine **dünne Delegations-Schicht**, die `OllamaClient` kapselt und das `TextEmbeddingEngine`-Trait für Tauri implementiert.
...
- **Maßnahme**: Alle Such-Invocations wurden konsistent (gemäß APM-6 Sibling-Konsistenz) auf das empfohlene `Collection::query()` Fluent Builder-Muster umgestellt:
  ...
  collection
      ...
      .await?
  ```
- **Verifikation**: `cargo clippy -p contextra-tauri --all-targets --no-deps -- -D warnings` verläuft mit 0 Fehlern und 0 Warnungen.
...
- **Verifikation**: `cargo clippy -p contextra-tauri --all-targets --no-deps -- -D warnings` (0 Warnungen) und `cargo test -p contextra-tauri --all-features` (83/83 Tests grün).

---

## 13. Session-Verifikation & Zustand-Bestätigung (TS: 2026-09-02T08:20:09Z) (SESSION: ce53c24a)

- **Audited Components**: `contextra-tauri` (`pdf.rs`, `docx.rs`, `email.rs`, `pipeline.rs`, `commands/mod.rs`, `commands/ingest.rs`, `state.rs`, `ollama.rs`, `lib.rs`).
- **Verifikations-Ergebnisse**:
  - `cargo check -p contextra-tauri --all-features` -> 0 Fehler, 0 Warnungen.
  - `cargo clippy -p contextra-tauri --no-deps -- -D warnings` -> 0 Findings.
  - `cargo fmt --check -p contextra-tauri` -> 0 Diffs.
  - `cargo test -p contextra-tauri --all-features` -> 83/83 Tests grün (0 failed).
...
### Inventar-Drift
- **Befund (Schritt 0)**: Das Prompter-Inventar vom 2026-09-03 verzeichnete 6 Dateien (`lib.rs`, `state.rs`, `ingestion/pdf.rs`, `docx.rs`, `email.rs`, `ollama.rs`). Der reale Repository-Zustand umfasst **17 Rust-Quellcodedateien** in `crates/contextra-tauri/src/`.
...
- **Auswirkung**: `cargo check -p contextra-tauri` schlägt fehl (`error[E0063]: missing field 'provenance' in initializer of 'SearchResultDto'`).
- **Maßnahme**: Inline-Befund `AI-TAG[BLOCKER][BUG]` mit ID `AGT-TAURI-96c44961` in `crates/contextra-tauri/src/commands/search.rs` hinterlegt. Der funktionale Fix ist gemäß Auditor-Rollensperre Gegenstand eines separaten Fix-Auftrags.
