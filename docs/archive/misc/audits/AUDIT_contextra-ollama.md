# Audit Report: `contextra-ollama` Security & HTTP Robustness Audit

**Date:** 2026-08-30
**Target Crate:** `contextra-ollama` (v0.1.0)
...
According to **ADR-008**, `contextra-ollama` serves as the primary LLM and embedding integration backend for Contextra, replacing the in-process ONNX baseline for main operations. Because `contextra-ollama` handles the only network I/O boundary in the core Contextra stack, its resilience against network failure, latency anomalies, malformed LLM responses, and prompt injection attacks is critical.
...
| **Empty / Whitespace Input** | `""` or `"   "` | Handled cleanly; rejected with `ContextraError::InvalidInput` where applicable. | **YES** |
...
| :--- | :--- | :--- | :--- |
| **a) Connection Refused** | Target `http://127.0.0.1:1` | Returns `ContextraError::Io` (ConnectionRefused). Retries up to `max_retries`. | PASS |
| **b) Timeout** | Response delayed > 50ms (timeout configured to 50ms) | Returns `ContextraError::Io` (TimedOut) after max retries exhausted. | PASS |
| **c) HTTP 4xx/5xx Codes** | HTTP 503 Service Unavailable / HTTP 400 Bad Request | 503 triggers retries; 400 returns immediate `ContextraError::InvalidInput` (no retry). | PASS |
| **d) Malformed JSON** | Server returns `{ malformed json payload ...` | Returns `ContextraError::Internal("JSON parse error")`. No panic. | PASS |
| **e) Missing Schema Fields** | Response missing expected `"response"` key | Returns `ContextraError::Internal("missing message.content")`. Controlled failure. | PASS |
...
- **Empty Input Safety:** `generate_prefix()` returns `ContextraError::InvalidInput` when provided empty document or chunk strings.
...
- **Score Bounds:** Rejects responses without valid float representations, returning `ContextraError::Internal`.
- **Input Validation:** Rejects empty or whitespace-only chunk texts with `ContextraError::InvalidInput`.

---

## 7. Embedder Consistency Results

- **Dimension Consistency:** `OllamaEmbedder` verifies embedding dimensions against `known_dimension()` (e.g., 768 for `nomic-embed-text`), returning `ContextraError::Index` on dimension mismatch to signal re-indexing requirement.
...
Benchmarks executed via `criterion` v0.5 (`crates/contextra-ollama/benches/ollama_bench.rs`):
...
| **BUG-OLL-02** | Low | Testing | Missing `proptest` and `criterion` targets in `contextra-ollama` manifest. | **RESOLVED** (Target added) |
...
- **Streaming NDJSON Line Buffer Hardening (`chat_with_rag_streaming`)**: Hardened stream chunk parsing by introducing an internal `line_buffer: Vec<u8>` for byte stream chunks. This prevents `ContextraError::Serialization` failures when JSON lines are split across TCP chunk boundaries. Added unit test `test_chat_with_rag_streaming_split_chunks`.

## 13. Audit Update: 2026-09-02

- **Importance Scoring Regex Edge Case Hardening (`score_importance`)**: Verified regex matching and float parsing across multiple LLM response formats. Added unit test `test_score_importance_regex_parsing_edge_cases` in `crates/contextra-ollama/src/importance.rs`.
...
- 75 Unit- & Integrationstests in `contextra-ollama` erfolgreich bestanden (0 FAILED).
...
- `#![forbid(unsafe_code)]` compliance verified across `crates/contextra-ollama` (0 unsafe blocks).
...
- `REVIEW-PASS[1/2]` in `crates/contextra-ollama/src/lib.rs` gesetzt.
