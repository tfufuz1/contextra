...
### `crates/contextra-db/src/collection/mod.rs`
```rust
...
- In `crates/contextra-db/src/collection/tests.rs` wurden bei `test_collection_with_diskann_index_hybrid_search` die Modulqualifikationen für `super::StoredDocument` und `super::StoredDocumentMeta` bereinigt, um die Instanziierung von `Collection::<LsmStorage, DiskAnnIndex>::new(...)` sauber im Test harness auszuführen.

---

## 3. Feature-Flag Verifikation (`experimental-diskann`)

- `crates/contextra-db/Cargo.toml` verdrahtet das Feature wie folgt:
  ```toml
  [features]
...
```
$ cargo test -p contextra-db --features experimental-diskann test_collection_with_diskann_index_hybrid_search -- --nocapture
...
Vollständiger Check:
- `cargo check --workspace --exclude contextra-tauri`: OK
- `cargo check -p contextra-db`: OK
- `cargo check -p contextra-db --features experimental-diskann`: OK
- `cargo clippy -p contextra-db -- -D warnings`: OK
- `cargo test -p contextra-db`: OK (141 unit tests + all integration test suites passed)
