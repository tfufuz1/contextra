...
1. **Automatisierte Trend-Analyse (`xtask check-unwrap-baseline-trend`)**:
   - Vergleicht bei jedem CI-Lauf den Stand von `.unwrap-baseline.json` des PR-Branches mit dem Base-Branch (`CONTEXTRA_CI_BASE_REF` / `origin/main`).
...
2. **Kontinuierliche Historisierung (`docs/unwrap_baseline_history.jsonl`)**:
   ...
   - Format: `{"date": "YYYY-MM-DD", "commit": "<hash>", "total": N, "by_tier1_crate": {"contextra-core": N1, ...}}`
...
*Crates:* `contextra-core`, `contextra-crypto`, `contextra-store`, `contextra-index`, `contextra-db`, `contextra-py`, `contextra-mcp`

- **Risiko:**
  - `contextra-core`, `contextra-store`, `contextra-index`: Lock-Poisoning-Kaskaden in Multi-Threaded Execution, Dateninkonsistenz in MVCC/LSM structures.
  - `contextra-crypto`: Sicherheitskritscher Code, Timing-Lecks, unkontrollierte Panics bei Schlüsselerzeugung/Integritätsprüfung.
  - `contextra-py`: FFI-Grenze. Panics an PyO3-Grenzen führen zum abrupten Prozessabsturz (SIGABRT) des Python-Interpreters.
  - `contextra-mcp`: Stdio JSON-RPC Server. Panics unterbrechen die MCP-Protokollverbindung.
- **Zielvorgabe:** Priorisierter Abbau ab Tag 1.

### Tier 2: Logik- & Integrationskomponenten (Mittlere Priorität)
*Crates:* `contextra-graph`, `contextra-text`, `contextra-checkpoint`, `contextra-agent`, `contextra-tauri`
...
*Crates:* `contextra-embed`, `contextra-ollama`, `contextra-router`, `contextra-calibration`, `contextra-candle`, `contextra-kv-bridge`, `xtask`, `benchmarks`
...
- **Wöchentliche Nettoreduktion Tier-1:** mindestens **−20 Einträge / Woche** in Tier-1-Crates (beginnend mit `contextra-core` und `contextra-crypto`).
- **Quartalsziel (Q4):** Reduktion der Gesamtzahl von 4.486 auf **< 4.000 Einträge** (entspricht ~10% Reduktion).
- **Hard Limit an FFI-/Sicherheits-Grenzen:** **0 Unwraps** in `contextra-py` (Python FFI) und `contextra-crypto` außerhalb dedizierter `#[cfg(test)]`-Blöcke.
...
### Muster 1: Fehlerfortpflanzung mittels `?` und `ContextraError`
```rust
// Vorher (Gefährlich)
let value = map.get("key").unwrap();

// Nachher (Sicher)
let value = map.get("key").ok_or_else(|| ContextraError::InvalidInput("Key 'key' not found".into()))?;
```
...
// Nachher (Sicher)
let guard = self.lock.lock().map_err(|_| ContextraError::InternalError("Lock poisoned".into()))?;
```
...
// Nachher
static RE: OnceLock<Result<Regex, ContextraError>> = OnceLock::new();
