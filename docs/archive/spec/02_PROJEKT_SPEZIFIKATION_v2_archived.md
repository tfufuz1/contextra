# Contextra — Projekt-Spezifikation (v2, erweitert)

**Verifiziert gegen:** frischer `git clone https://github.com/tfufuz1/contextra`, HEAD `33226e2` (09.09.2026, im Rahmen dieser Session).
...
| 2 | Neuer Abschnitt 6: mikrofeingranulare Schnittstellenspezifikation für alle 17 Workspace-/FFI-Crates (Modul-Karten, Kern-Traits mit exakten Signaturen, kritische Invarianten, Cross-Crate-Kanten). | `crates/*/AGENTS.md`, `crates/contextra-core/src/traits/mod.rs` |
| 3 | Neuer Abschnitt 7: Statusmatrix der zehn seit v1 laufenden Härtungs-Fix-Prompts (N1–N5, B, E1/E2, D1/D2, F/G/H), verifiziert gegen den tatsächlichen HEAD-Code, nicht gegen die Prompt-Absicht. | `Contextra_Jules_Fix_Prompts_*.md`, Live-Grep gegen HEAD |
| 4 | Dokumentierte Namens-/Pfad-Drift: `AGENTS.md` (root, Stand `b448084`) referenziert weiterhin `crates/contextra-kv-bridge/` als eigenständiges Crate; im tatsächlichen Code ist dieser Bestandteil seit mehreren Sessions in `crates/contextra-crypto/src/kv_segment/` konsolidiert. `WORKING_STATE.md` (autogeneriert!) listet das Crate zudem unter dem Namen `contextra-security` statt `contextra-crypto`. Beide Docs sind zum jeweiligen Verifikationszeitpunkt intern inkonsistent — ein konkretes, im Live-Repo nachvollziehbares Beispiel für die in `03_ENTWICKLUNGSSYSTEM_SPEZIFIKATION.md` §3.2 beschriebene Doku-Drift-Gate-Lücke (Gate 5 prüft Sync gegen `WORKING_STATE.md`/`ARCHITECTURE.md`/`SOURCE_OF_TRUTH.md`, aber nicht gegen `AGENTS.md` selbst). | `AGENTS.md` §2 vs. `find crates/`, `WORKING_STATE.md` Crate-Inventar-Tabelle |
...
Contextra ist eine **eingebettete, kryptographisch gehärtete AI-Memory-Infrastruktur in Rust** — eine lokale, air-gapped-fähige Speicher- und Retrieval-Engine für LLM-Agenten, die Langzeitgedächtnis, Graph-basiertes Retrieval und Mandanten-Isolation kombiniert. Kein Cloud-Dienst, keine Server-Farm-Architektur: Das System ist als **Library** konzipiert, die in andere Anwendungen eingebettet wird.

### 1.1 Produktvision (ADR-077, verbindlich seit 08.09.2026)

Nach einer Phase, in der drei Produktvisionen parallel verfolgt wurden (PyPI-Library / Desktop-Enterprise-App „Contextra Brain" via Tauri / Voice-Assistant „Jarvis"), ist die Vision seit ADR-077 **formal entschieden**:

- **Primär: PyPI-Library** (`contextra-py`) — Positionierung als „schlanker als MinnsDB, mit kryptographischer Härtung als Alleinstellungsmerkmal". Zielgruppe: Entwickler, die ein eingebettetes, auditierbares Memory-Substrat für eigene Agenten-Anwendungen brauchen, ohne Cloud-Abhängigkeit.
- **Deprecated, in Entfernung:** Die Tauri-Desktop-App (`contextra-tauri`, „Contextra Brain") — ehemals Positionierung als air-gapped Unternehmensassistent für DACH-Marktsegment. ADR-018 (Doppelstrategie) ist durch ADR-077 abgelöst. **Zielfrist der physischen Entfernung: ≈ 07.11.2026** (in v1 als Lücke benannt; zum Zeitpunkt von v2 weiterhin nicht durch ein CI-Gate überwacht, `crates/contextra-tauri` existiert unverändert, `.github/workflows/tauri-release.yml` läuft weiterhin).
...
- **Mandanten-Isolation (`TenantId`, `TenantIsolatedKvStore`):** Strikte kryptographische Trennung zwischen Mandanten-Kontexten (Krypto-/Isolationsschicht implementiert, siehe `AUDIT_contextra-kv-bridge.md`; Ende-zu-Ende-Bridge in Inferenz-Engine in Bearbeitung), inklusive eines formalen, permanenten Vetos (`VETO-F10`) gegen jede Form von „Cross-Tenant Knowledge Sharing". Seit v1 zusätzlich in Härtung: tenant-faire LRU-Eviction (`evict_lru_fair()`, Prompt-Vorläufer bereits **gemerged**, Commit `b5a68b9`), Nachfolge-Härtung (Sichtbarkeitsreduktion von `evict_lru_global()`, Rotations-Bias-Fix, Lock-Batching — Prompts E1/E2) **noch nicht gemerged**.

---

## 2. Architektur: Crate-Topologie (Layer 0–6, DAG-Modell)

Contextra ist strikt als gerichteter azyklischer Graph (DAG) von Cargo-Workspace-Crates organisiert. Jeder Layer darf nur auf Layer mit niedrigerer Nummer verweisen — Zyklen und „Layer-Überspringen" werden durch das Gate `cargo xtask check-dag` in CI hart erzwungen.
...
|---|---|---|---|
| **0 — Fundament** | Kern-Typen, Traits, Fehlerbehandlung, Kalibrierungs-Grundbausteine | `contextra-core` (`TenantId`, `TxId`, `DocId`, `EntityId`, `ConfigFingerprint`, `ContextraError`, Kern-Traits), `contextra-calibration` (Platt/Isotonic-Scaler) | core: 9.625 · calibration: 1.282 |
| **1 — Storage-Primitiven & Vertikalen** | Verschlüsselung, KV-Sicherheitsschicht, Checkpointing, Graph-Kern, Volltext, native Inferenz | `contextra-crypto` (Encryption-at-Rest, `DeletionProof`, **inkl. konsolidiertem KV-Segment-Store, ehem. `contextra-kv-bridge`**), `contextra-checkpoint`, `contextra-graph` (CSR-Graph, `ConsistencyEnforcer`, `PathRAGEngine`, `EdgeProvenance`), `contextra-text` (BM25, DACH-Kompositum-Splitting), `contextra-candle` (natives GGUF-Inferenz-Backend) | crypto: 2.887 · checkpoint: 2.666 · graph: 9.189 · text: 3.994 · candle: 1.216 |
| **2 — Subsysteme** | Vektorsuche, Embedding-Provider, Storage-Engine | `contextra-embed` (`CrossEncoderReranker`, optional, Feature `onnx`), `contextra-index` (HNSW, SQ8-Quantisierung, DiskANN hinter `experimental-diskann`), `contextra-ollama` (`ContextPrefixEngine`), `contextra-store` (LSM-Tree, WAL) | embed: 1.674 · index: 10.666 · ollama: 3.925 · store: 11.570 |
| **3 — Hauptdatenbank** | Kollektions-CRUD, 4-Signal-Fusion, Konsolidierung | `contextra-db` (`ConsolidationSession`, `AdaptiveDecayController`/F-01, `ConsolidationEngine`, `MarkdownChunker`, `MultiStepEngine` mit RRF-Fusion) | db: 19.686 |
| **4 — Orchestrierung & Frontend** | Benchmark-Harness, Routing, Desktop-Shell | `contextra-bench` (nicht Teil dieser Spezifikation im Detail), `contextra-router` (Conformal-Routing, SLM-Profile), `contextra-tauri` (Deprecated, ADR-077) | router: 4.838 · tauri: 4.340 |
| **5 — Agenten-Engine** | Persistente Agenten-Workflows | `contextra-agent` (`PersistentAgentWorkflow`, State-Graph + Checkpointing) | agent: 2.757 |
| **6 — Protocol & Sandbox** | Sichere Bedienoberflächen für externe Agenten | `contextra-mcp` (`McpSandbox`, Read-Only + Write-Authorization-Guard) | mcp: 3.175 |
| **Außerhalb des Root-Workspace (ADR-064)** | Python-FFI-Bindung | `contextra-py` (`panic = "unwind"`, separater Build) | py: 1.617 |

**Wichtige strukturelle Sonderregel:** `contextra-py` ist **bewusst nicht** im Root-`Cargo.toml`-Workspace `members`-Array eingebunden (ADR-064, verifiziert: 18 Members im Workspace-Array, `contextra-py` fehlt). Grund: `contextra-py` benötigt `panic = "unwind"` im Release-Profil, damit `catch_unwind()` an der FFI-Grenze zu Python funktioniert — der restliche Workspace nutzt `panic = "abort"` (kleinere Binaries, kein Unwind-Overhead). Diese Trennung ist dokumentierte Absicht, keine technische Schuld.
...
- **2-Phase-Commit über 4 Indizes** (`contextra-db`), WAL-Format V3 mit `tx_id`-HMAC-Binding.
...
- **`commit_mutex → state`-Lock-Hierarchie** (`contextra-store::lsm::Lsm::commit()`): `commit_mutex` wird über die gesamte Funktionsdauer inklusive Fehler-/Rollback-Pfad gehalten; die Invariante ist Stand HEAD `33226e2` durch einen dedizierten Concurrency-Regressionstest verifiziert (Prompt N4), aber — im Gegensatz zum in Prompt N5 vorgeschlagenen Refactor — **weiterhin nur durch Konvention, nicht durch das Typsystem** gesichert (`rollback_to_tx_locked()` verlangt noch keinen Compiler-geprüften `CommitGuard`-Beweis; Prompt N5 hat niedrigste Priorität und ist optional).
...
- **BM25** (`contextra-text`) mit DACH-spezifischer Kompositum-Zerlegung.
...
- **Kaskadierende CSR-Invalidierung** — `DocEdgeIndex`-basiertes Tombstoning verknüpfter Graph-Kanten bei Dokument-Superseding (`crates/contextra-db/src/collection/crud.rs:958`).

### 3.4 Mandanten-Isolation & Sicherheit
- **`TenantId`** (Layer 0, `contextra-core::types::domain::TenantId(pub u64)`), durchgezogen bis in `TenantIsolatedKvStore` (Layer 1, `contextra-crypto::kv_segment::store`).
...
- **`McpSandbox`** — Read-Only-MCP-Server-Sandbox mit explizitem Write-Authorization-Guard; MCP läuft laut ADR-010 bewusst als reines `stdio`-Protokoll ohne HTTP-Framework — Gate 4 in `context-gates.yml` erzwingt das aktiv (`grep` gegen `Cargo.toml` von `contextra-mcp` auf `axum`).

### 3.5 Agenten-Ausführung
- **`PersistentAgentWorkflow`** (`contextra-agent`) — Multi-Step-Agent-Execution-Loop mit State-Graph und Checkpointing, Zustandsautomat `Pending → Running → Suspended/Completed/Failed`, ausschließlich durch `OrchestratorEngine` transitionierbar.
...
- **Desktop-Enterprise-App als primärer Vertriebskanal** — durch ADR-077 abgelöst; verbleibender Code (`contextra-tauri`) befindet sich weiterhin im Abbau, physische Entfernung zum Zeitpunkt von v2 noch nicht erfolgt.
...
### 6.0 Layer 0 — `contextra-core`: Kern-Trait-Contracts (verbindlich für alle Implementoren)

`contextra-core` enthält **kein I/O, kein async-Runtime-Binding, kein Netzwerk** in den Typ-Modulen — ausschließlich reine Datenstrukturen und Trait-Contracts (`crates/contextra-core/src/traits/mod.rs`, 1.813 LOC; `crates/contextra-core/src/traits/embedding.rs`, 174 LOC).

**Domänen-Newtypes** (`crates/contextra-core/src/types/domain.rs`):
```rust
...
**`StorageEngine`-Trait** (einziger Implementor: `contextra-store::LsmStorage`):
```rust
...
**`VectorIndex`-, `TextIndex`-, `GraphIndex`-Traits** (Implementoren: `contextra-index::HnswIndex`, `contextra-text::InvertedIndex`, `contextra-graph`) folgen demselben Muster: `insert`/`insert_batch`, `search`/`search_at`/`search_filtered` (MVCC-fähig via Sequenznummer), `delete`, `commit`/`rollback`/`rollback_to_tx`, `last_tx_id`, `len`/`is_empty` (Default-Impl über `len`), `stats`. `GraphIndex` ergänzt `traverse`/`traverse_at`/`traverse_at_time`/`traverse_at_bitemporal` (bi-temporale Achsen, ADR-033), `personalized_page_rank`, `add_entity`/`add_edge`/`add_bidirectional`/`remove_edge`, `multi_traverse`.
...
**`ContextraError`** (`crates/contextra-core/src/error.rs`) — **die einzige Fehler-Enum im gesamten Workspace**, append-only für Binärkompatibilität, `From`-Impls ausschließlich in `error.rs`:
```rust
...
**Kritische Invarianten (contextra-core):**
...
### 6.1 `contextra-store` (Layer 1, LSM-Tree Storage Engine)
> ~11.570 LOC · 10 Dateien · einziger Implementor von `StorageEngine`

**Modul-Karte:** `lsm.rs` (`LsmStorage` — Orchestrator, implementiert `StorageEngine`) · `wal.rs` (Write-Ahead-Log, HMAC-Chaining, CRC32/Entry) · `memtable.rs` (Skip-List mit Sequenznummern, Tombstones) · `sstable.rs` (Block-komprimierte On-Disk-Segmente, Bloom-Filter, CRC32) · `compaction.rs` (`CompactionEngine`, Tiered/Leveled-Merge) · `checkpoint.rs` (`pub(crate)`, internes MVCC-Pinning — **nicht** die öffentliche API von `contextra-checkpoint`) · `mmap.rs` (Memory-Mapped-Utilities) · `util.rs` (`pub(crate)`, u. a. `load_or_create_integrity_key`).
...
**Cross-Crate-Kanten:** Implementiert `contextra-core::StorageEngine`; wird von `contextra-db`, `contextra-text`, `contextra-checkpoint` (intern) konsumiert.

---

### 6.2 `contextra-index` (Layer 2, HNSW/DiskANN Vektorsuche)
...
**Cross-Crate-Kanten:** Implementiert `contextra-core::VectorIndex`; konsumiert von `contextra-db` (Signal 1 der 4-Signal-Fusion).

---

### 6.3 `contextra-db` (Layer 3, Hauptdatenbank / Collection API)
...
**Cross-Crate-Kanten:** Konsumiert `contextra-store`, `contextra-index`, `contextra-text`, `contextra-graph`, `contextra-embed`/`contextra-ollama` (Embeddings); wird konsumiert von `contextra-py`, `contextra-agent`, `contextra-mcp`, `contextra-tauri`.

---

### 6.4 `contextra-crypto` (Layer 1, Encryption-at-Rest, HMAC, Zeroize, KV-Segment-Store)
> ~2.887 LOC · 11 Dateien · **enthält seit Konsolidierung auch den ehemaligen `contextra-kv-bridge`-Bestandteil** (`src/kv_segment/`; Krypto-/Isolationsschicht implementiert, siehe `AUDIT_contextra-kv-bridge.md`; Ende-zu-Ende-Bridge in Bearbeitung)
...
- **HMAC Chaining (WAL):** Bruch der Kette → `ContextraError::WalCorruption`, hartes Abbrechen.
...
**Cross-Crate-Kanten:** `contextra-store` (WAL-Verschlüsselung), `contextra-index`/`contextra-graph` (KV-Cache-Segmente), Aufrufer von `DeletionProof::create()` verteilt über mehrere Crates (genaue Liste ist Teil der Prompt-D1-Analysepflicht, s. §7).

---

### 6.5 `contextra-graph` (Layer 1, CSR-Graph, Entity-Relation-Traversal)
...
**Cross-Crate-Kanten:** Implementiert `contextra-core::GraphIndex`; konsumiert von `contextra-db` (Signal 3 der 4-Signal-Fusion).

---

### 6.6 `contextra-text` (Layer 1, BM25 Volltextsuche)
...
**Cross-Crate-Kanten:** Implementiert `contextra-core::TextIndex`; konsumiert von `contextra-db` (Signal 2 der 4-Signal-Fusion).

---

### 6.7 `contextra-checkpoint` (Layer 1, Snapshot-Pinning, RAII-Transaktions-Guards)
...
**Cross-Crate-Kanten:** Konsumiert `contextra-core::Checkpoint`/`CheckpointCoordinator`, `contextra-store` (Pinning).

---

### 6.8 `contextra-calibration` (Layer 0, Kalibrierungsprimitive)
> ~1.282 LOC · 5 Dateien

**Kern-API:** `IsotonicCalibrator`, `PlattScaler`, `invalidate_on_config_change(new_fingerprint: ConfigFingerprint)`.

**Kritische Invarianten:** Layer-0-Platzierung (hängt nur von `contextra-core` ab); `#![forbid(unsafe_code)]`; P8-Compliance (Kalibrierung MUSS bei Konfigurationsänderung via `ConfigFingerprint` invalidiert werden).

**Cross-Crate-Kanten:** Konsumiert von `contextra-router`, `contextra-embed`, `contextra-db`.

---

### 6.9 `contextra-candle` (Layer 1, natives GGUF-Inferenz-Backend)
...
### 6.10 `contextra-embed` (Layer 2, ONNX-Embeddings & Reranking, optional)
...
### 6.11 `contextra-ollama` (Layer 2, Ollama-HTTP-Client)
...
### 6.12 `contextra-mcp` (Layer 6, Model Context Protocol Server)
> ~3.175 LOC · 7 Dateien
...
- **Sandbox-Defaults:** `allow_db_reads: true`, `allow_db_writes: false` (Opt-in via `CONTEXTRA_MCP_WRITE_ALLOW`), `allow_code_execution: false` (strikt).
...
### 6.13 `contextra-agent` (Layer 5, Agenten-Orchestrierung)
> ~2.757 LOC · 8 Dateien
...
### 6.14 `contextra-router` (Layer 4, SLM-Routing, Conformal Calibration)
> ~4.838 LOC · 8 Dateien
...
### 6.15 `contextra-py` (außerhalb Root-Workspace, PyO3-FFI, ADR-064)
> ~1.617 LOC · 1 Datei

**Modul-Karte:** `lib.rs` (`PyContextra`, `PyCollection`) · `types.rs` (NumPy/Dict/String-Konvertierung) · `error.rs` (FFI-Error-Mapping) · `gil.rs` (GIL-Management).

**Kritische Invarianten:**
- **GIL-Release-Protokoll (AGT-PY-001):** Blockierende/asynchrone Contextra-Aufrufe MÜSSEN in `py.allow_threads(|| { ... })` gewrappt werden.
...
- **Separater Build:** `cd crates/contextra-py && cargo build --release` (abweichendes Panic-Profil, s. Abschnitt 2).

---

### 6.16 `contextra-tauri` (Layer 4, Desktop-Backend, DEPRECATED via ADR-077)
...
- **IPC-Command-Protokoll:** Alle `#[tauri::command]`-Funktionen mit I/O müssen async sein; Fehler MÜSSEN in `ContextraErrorDto` gemappt werden.
...
|---|---|---|---|---|---|
| **N1** | RRF-Fusion: NaN/Inf-Guard für `weight`/`doc.score` | `contextra-db/src/fusion.rs` | Hoch | ✅ **Gemergt** (`1fedb8d`) | `!weight.is_finite()`-Guard verifiziert im Code |
| **N2** | WAL `truncate()`: atomare `size`/`last_hmac`-Aktualisierung | `contextra-store/src/wal.rs` | Mittel | ✅ **Gemergt** (`59ce141`) | Commit-Message „close TOCTOU window during WAL truncation" |
| **N3** | LSM: Drift-Zähler für fehlschlagendes `budget.consume_memory()` | `contextra-store/src/lsm.rs` | Mittel | ✅ **Gemergt** (`4dff111`) | `budget_tracking_drift_bytes`-Atomic-Metrik verifiziert |
...
| **N5** | Typsystemische Erzwingung `commit_mutex → state` (optional) | `contextra-store/src/lsm.rs` | Niedrig | ✅ **Gemergt** (`de319a4`) | Commit „enforce commit_mutex for rollback_to_tx_locked" |
| **B** | Verpflichtendes Limit für `scan()`/`scan_prefix()` | `contextra-db`, `contextra-py` | Hoch | ✅ **Gemergt, abweichend implementiert** (`f29c398`) | `MAX_SCAN_RESULTS`-Konstante als stiller Cap statt `LimitExceeded`-Fehler (s. §3.6) |
| **E1** | Härtung `evict_lru_fair()`: toter Pfad, Fairness-Bias | `contextra-crypto/src/kv_segment/store.rs` | Mittel | ⏳ **Offen** | `evict_lru_global()` weiterhin `pub`, kein `eviction_round_offset`-Feld im Code gefunden |
| **E2** | Lock-Granularität `evict_lru_fair()` (Batch-Freigabe) | `contextra-crypto/src/kv_segment/store.rs` | Mittel | ⏳ **Offen** (setzt E1 voraus) | Kein `MAX_ROUNDS_PER_LOCK_ACQUISITION` im Code gefunden |
| **D1** | Typsystemische Erzwingung `DeletionProof::create()`-Vorbedingung | `contextra-crypto/src/deletion_proof.rs` + Call-Sites | Hoch (Compliance) | ⏳ **Offen** | Kein `LayerCleanupProof`-Typ im Code gefunden |
| **D2** | NaN/Inf-Guard im DiskANN-Robust-Pruning | `contextra-index/src/diskann.rs` | Mittel | ⏳ **Offen** | Kein `is_finite()`-Check in den referenzierten α-Pruning-Zeilen gefunden |
| **F** | Speicherbeschränkter Range-Scan `scan_bounded` | `contextra-core`, `contextra-store`, `contextra-db` | Hoch (OOM) | ⏳ **Offen** (setzt B voraus, erfüllt) | Keine `scan_bounded`-Trait-Methode in `contextra-core::traits::mod` gefunden |
| **G** | DiskANN `search_internal()`: Query-Vektor-Validierung | `contextra-index/src/diskann.rs` | Niedrig-Mittel | ⏳ **Offen** | Kein `query.iter().all(\|v\| v.is_finite())`-Check gefunden |
| **H** | Nachtrag N1: `HeapEntry` auf `total_cmp`, Release-Sichtbarkeit `INV-PROV-1` | `contextra-db/src/fusion.rs` | Mittel | ⏳ **Offen** (setzt N1 voraus, erfüllt) | `impl Ord for HeapEntry` nutzt weiterhin `partial_cmp().unwrap_or(Ordering::Equal)` |
