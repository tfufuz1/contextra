...
**Date:** 2026-09-12
**Crate:** `contextra-index` (Layer 2 — Vector Search Engine)
...
| **5. Cross-Crate Entry Point Wiring** | **PASS** | Query vector `is_finite()` validation is centralized in `HnswIndex::search_filtered_internal` and `DiskAnnIndex::search_internal`. All public and internal search paths (`search`, `search_filtered`, `search_at`) called from `contextra-db` are 100% guarded against NaN/Inf and oversized `k`. |
...
- **`HnswConfig::validate()` Enforcement:**
  - `dimension == 0` -> `Err(ContextraError::InvalidInput)`
  - `m == 0` -> `Err(ContextraError::InvalidInput)`
  - `ef_search == 0` -> `Err(ContextraError::InvalidInput)`
  - `ef_construction < m` -> `Err(ContextraError::InvalidInput)` (Invariant `INV-HNSW-1`)
  - `rebuild_threshold` outside `0.0..=1.0` -> `Err(ContextraError::InvalidInput)`
...
- **`HnswIndex`:**
  ...
  - `search_filtered_internal()` validates:
    ...
    2. `k > MAX_SEARCH_K` (10,000) returns `ContextraError::InvalidInput`.
    3. `query.iter().all(|v| v.is_finite())` returns `ContextraError::InvalidInput` if any component is NaN or Infinity.
- **`DiskAnnIndex`:**
...
- **Cross-Crate Call-Site Verification:**
  - All calls in `contextra-db` (`collection/search.rs`, `lib.rs`, `multistep.rs`) pass through `VectorIndex::search`, `search_filtered`, or `search_at`.
...
1. **`JULES-20260912-INDEX-01` (P3 / Maintenance):** Clean up `cfg(feature = "graph")` warning in `crates/contextra-index/src/lib.rs` by adding `graph` feature or removing obsolete conditional export.
...
**VERDICT: GO / APPROVED**
`contextra-index` complies 100% with all audit criteria, SIMD determinism standards, NaN/Inf arithmetic guard completeness, zero-downtime CoW persistence guarantees, and cross-crate entry point safety directives.
