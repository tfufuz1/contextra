# AUDIT Report: WAL Durability & Flusher Architecture (`contextra-store`)

## Metadaten & Scope
* **Ziel-Crate:** `crates/contextra-store` (Layer 1)
* **Auditierte Dateien:**
  * `crates/contextra-store/src/wal/flusher.rs`
  * `crates/contextra-store/src/wal/segment.rs` (Dateiexistenz geprüft — Datei existiert nicht im Repository)
* **Modus:** `audit-readonly`
* **Claim-Status:** Registriert via `cargo xtask claim --crate contextra-store --mode audit-readonly`
...
* **Status:** **BELEGT**
* **Exakter Codepfad:** `crates/contextra-store/src/wal/flusher.rs:182-231`
...
* **Status:** **BELEGT**
* **Exakter Codepfad:** `crates/contextra-store/src/wal/flusher.rs:182-231` sowie `crates/contextra-store/src/wal/io.rs:504-508`

### Analyse
* **Fehlerumwandlung:** Jeder `std::io::Error` bei `write_all`, `flush` oder `sync_all` wird sofort über `.map_err(...)` abgefangen und in einen `ContextraError::Storage(...)` konvertiert:
  ```rust
...
* **Client-Aufruf-Sicht:** In `Wal::append_batch_locked` wartet der Aufrufer auf die ACK-Oneshot-Response:
  ...
  ```
  (`io.rs:504-508`). Durch den doppelten `?`-Operator wird der `ContextraError::Storage` direkt an den Caller zurückgegeben.
...
* **Status:** **BELEGT** (in `flusher.rs` und `io.rs`) / **NICHT VERIFIZIERBAR** (bezüglich `crates/contextra-store/src/wal/segment.rs`)

### Dateiexistenz-Hinweis
Die Datei `crates/contextra-store/src/wal/segment.rs` existiert **nicht** im Repository. Segment-Versiegelung und -Rotation sind direkt in `flusher.rs` (`WalCommand::Seal`) und `io.rs` (`Wal::rotate_and_seal`) verankert.

### Rotations-Ablauf & Atomizität (`WalCommand::Seal`)
1. **Versiegelungs-Lock:** Beim Aufruf von `Wal::rotate_and_seal` wird die Atombeschreibung `sealed` auf `true` gesetzt (`io.rs:624`). Neue Schreibzugriffe (`append_batch` / `truncate`) werden sofort mit `ContextraError::Storage("Cannot append to sealed WAL segment...")` abgelehnt (`io.rs:430-434`, `io.rs:610-614`).
...
* **Status:** **BELEGT**
* **Exakter Codepfad:** `crates/contextra-store/src/wal/encode.rs:201-209` und `crates/contextra-store/src/wal/io.rs:183-200`
...
2. **Behandlung beim Replay Scan:** In `do_scan_entries_with_callback` (`io.rs:183-200`) wird der von `from_bytes` zurückgegebene Fehler bewertet:
   ...
   ```
   (`io.rs:191-200`).
3. **Kein Silent-Skip:** Bei einem CRC-Fehler an einer beliebigen Stelle in der Datei bricht das Replay **sofort mit `Err(ContextraError::WalCorruption)`** ab. Es wird nicht nur eine Warnung geloggt oder der Rest übersprungen.
...
* **Status:** **BELEGT**
* **Exakter Codepfad:** `crates/contextra-store/src/wal/flusher.rs:105` und `crates/contextra-store/src/wal/io.rs:498-508`
...
* **Scan-Scope:**
  * `crates/contextra-store/src/wal/flusher.rs`
  * `crates/contextra-store/src/wal/segment.rs`

### Befunde
* **`crates/contextra-store/src/wal/segment.rs`**: **Datei existiert nicht** im Repository.
* **`crates/contextra-store/src/wal/flusher.rs`**:
...
* **Status:** **BELEGT**
* **DAG-Integrität:** In `crates/contextra-store/Cargo.toml` sind ausschließlich Abhängigkeiten zu Layer-0-Crates (`contextra-core`, `contextra-crypto`) deklariert. Es existieren keinerlei Importe aus höheren Layern (`contextra-db`, `contextra-candle`, `contextra-router`, `contextra-index`). Die Schichtenarchitektur (DAG) ist eingehalten.
