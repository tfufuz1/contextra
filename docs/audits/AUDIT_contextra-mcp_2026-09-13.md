...
| `contextra_insert` | `metadata` | Object | Valid JSON Object | `v.as_object()` | **Unbounded Metadata Payload Size** (durch 4MB RPC Line beschränkt) | Niedrig |
| `contextra_get` | `id` | String | Non-empty, max 256 Chars | `s.trim().is_empty()` | **LÜCKE**: Keine explizite Prüfung `id.len() <= 256` in `contextra_get` vor `col.get()` | Niedrig |
...
| :--- | :--- | :--- |
| **P2 (Mittel)** | **`contextra_get` ID-Längenbegrenzung**: Explizite Prüfung `id.len() <= 256` in `contextra_get` analog zu `contextra_insert` hinzufügen. | `crates/contextra-mcp/src/lib.rs` |
| **P2 (Mittel)** | **Homoglyph-Faltung im Guard**: Ergänzung einer Skeleton/ASCII-Confusable Normalisierung vor der Mustersuche in `normalize_text`. | `crates/contextra-mcp/src/prompt_injection.rs` |
| **P3 (Niedrig)** | **Read-Line Inactivity Timeout**: Schutz gegen unbegrenzte Slowloris-Connection-Holds durch ein tokio Timeout-Wrap um `read_line_bounded`. | `crates/contextra-mcp/src/lib.rs` |

---

## 6. Verifikation & Pre-Commit Status

- `cargo check -p contextra-mcp --all-features`: GRÜN
- `cargo test -p contextra-mcp --all-features`: 87/87 PASSED
- `cargo clippy -p contextra-mcp --all-features -- -D warnings`: GRÜN (0 Warnings)
- `cargo fmt --check -p contextra-mcp`: GRÜN
