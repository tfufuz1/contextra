# AUDIT: Fusion-Grenzflächen (HNSW / DiskANN) in `memfuse-index`

* **Crate:** `crates/memfuse-index` (Layer 3)
* **Audited Files:**
  * `crates/memfuse-index/src/hnsw.rs`
  * `crates/memfuse-index/src/diskann.rs`
  * `crates/memfuse-db/src/fusion.rs` (Konsument-Vergleich)
...
Dieses Audit untersucht die Schnittstellen und Invarianten der Vektorindex-Implementierungen (`HnswIndex` und `DiskAnnIndex`) an der Fusion-Grenzfläche zu `memfuse-db` (`fusion.rs`).
...
Die Indizes in `memfuse-index` implementieren das `VectorIndex`-Trait aus `memfuse-core::traits::vector_index` und stellen öffentliche Methoden zur Verfügung, die von `memfuse-db` (u.a. `Collection`, `DbTransaction` und Orchestrierung) konsumiert werden.

### 1.1 `HnswIndex` (`crates/memfuse-index/src/hnsw.rs`)
...
### 1.2 `DiskAnnIndex` (`crates/memfuse-index/src/diskann.rs`)
...
* In `fusion.rs` (`memfuse-db/src/fusion.rs:1-12`) wird **Reciprocal Rank Fusion (RRF)** eingesetzt:
...
* **Schlüsselrepräsentation**:
  * Es wird exakt derselbe `DocId`-Typ (`memfuse_core::DocId(u64)`) verwendet wie im Text-Inverted-Index (`memfuse-text`) und Graph-CSR-Index (`memfuse-graph`).
...
* **Soll-Hypothese:** `DiskAnnIndex::delete()` liefert unbedingt `MemFuseError::InvalidInput("DiskAnn is a read-only out-of-core index.")` zurück.
* **Ist-Befund im Code:** **WIDERLEGT**.
  * In `diskann.rs:1208-1236` ist die native Tombstone-Löschung für DiskANN **vollständig implementiert**:
    ...
    ```
  * Wenn das Dokument nicht im Index vorhanden ist, wird `MemFuseError::NotFound` zurückgegeben (`diskann.rs:1218`).
...
* Alle Grenzflächen-Funktionen nutzen die `Result<T, MemFuseError>`-Fehlerbehandlung.
...
Die Fusion-Grenzfläche zwischen `memfuse-index` und `memfuse-db/src/fusion.rs` ist sauber strukturiert:
