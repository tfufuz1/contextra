...
Das Crate `contextra-embed` stellt die In-Process-Embedding- und Cross-Encoder-Reranking-Funktionalität für das Contextra-Projekt bereit (Layer 3 im 8-Schichten-DAG). Gemäß **ADR-005 (Feature-Based Scaling)**, **ADR-008 (Embedding-Backend-Umstellung auf Ollama HTTP)** und der **Sovereign Core Doctrine (ADR-004)** ist das Crate so entworfen, dass der Standard-Build keinerlei ONNX-Runtime- oder Heavyweight-C++-Bibliotheken einbindet (`default = []`).

### Kernaussagen des Audits:
1. **Hermetische Feature-Gate-Isolation:** **PASSED**. Der Default-Build (`cargo check -p contextra-embed`) baut absolut sauber und isoliert ohne Verlinkung von `ort`, `tokenizers` oder `ndarray`. Downstream-Consumer ohne das `onnx`-Feature sehen keine ONNX-Typen in der öffentlichen API.
...
- Pre-RRF Oversampling Ceiling (`pre_rerank_k = k * 3`) in `contextra-db` fängt dies vor der Übergabe an den Reranker ab.
...
| **Tests** | `cargo test -p contextra-embed --all-features` | **33/33 Passed** |
| **Workspace Integrity** | `cargo check --workspace --exclude contextra-tauri` | **0 Errors** |
...
Das Crate `contextra-embed` ist in hervorragendem Zustand. Sämtliche ML-Scoring Invarianten, Concurrency-Anforderungen und Feature-Gate Isolationen sind vollständig verifiziert.
