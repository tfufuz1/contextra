# Audit-Report: `contextra-py` (Layer 3 — Python PyO3 Bindings)
...
`contextra-py` serves as the Layer 3 Python bridge for the Contextra embedded vector engine. It exposes high-performance hybrid search, CRUD, and graph relationship methods to Python via PyO3, NumPy, and FlatBuffers.
...
3. **Error Mapping Completeness**: `contextra_err` maps all `contextra_core::ContextraError` variants via `ContextraErrorDto` into PyO3 exception types (`PyKeyError`, `PyValueError`, `PyPermissionError`, `ContextraIOError`, `ContextraIndexError`, `ContextraCryptoError`, etc.) while attaching `kind`, `message`, and `details` attributes onto the Python exception instances.
...
- **#[forbid(unsafe_code)]**: `contextra-py` maintains a strict `#![forbid(unsafe_code)]` directive.
- **Shared Tokio Runtime**: A multi-thread Tokio runtime (`contextra-py-worker`) is lazily initialized via `OnceLock<Runtime>` in `get_runtime()`.

---

## Audit Verification & Test Delta (Session `JULES-20260911-DEEP`, TS: 2026-09-11T12:00:00Z)

- **Inventory Reality Check**: Confirmed `crates/contextra-py/src/lib.rs` matches actual repo inventory with 0 drift.
- **Rust Checks**: `cargo check --manifest-path crates/contextra-py/Cargo.toml --all-features` (0 errors, 0 warnings).
- **Clippy Analysis**: `cargo clippy --manifest-path crates/contextra-py/Cargo.toml -- -D warnings` (0 findings).
- **Formatting**: `cargo fmt --check --manifest-path crates/contextra-py/Cargo.toml` (0 diffs).
- **Rust Unit Test Suite**: `cargo test --manifest-path crates/contextra-py/Cargo.toml --all-features` (100% passed).
...
- **Inventory Reality Check**: Confirmed `crates/contextra-py/src/lib.rs` (1639 lines) matches actual repo inventory with 0 drift.
- **Rust Quality & Compilation**: `cargo check --manifest-path crates/contextra-py/Cargo.toml --all-features` (0 errors, 0 warnings).
- **Clippy Analysis**: `cargo clippy --manifest-path crates/contextra-py/Cargo.toml -- -D warnings` (0 findings).
- **Formatting**: `cargo fmt --check --manifest-path crates/contextra-py/Cargo.toml` (0 diffs).
...
- **Tier 1 Depth & Fault-Injection Verification**:
  ...
  - **Error Mapping & Attributes**: Verified structured `contextra_err` exception mapping with attached `kind`, `message`, and `details` fields.
...
- **Inventory Reality Check**: Confirmed `crates/contextra-py/src/lib.rs` matches actual repo inventory with 0 drift.
- **Input Guard Extension**: Added Rust unit tests for `validate_db_path`, `validate_query_text`, and `validate_batch_size` in `crates/contextra-py/src/lib.rs`.
- **Python FFI / Integration Test**: Added `test_long_query_text_validation` in `crates/contextra-py/tests/test_errors.py` verifying oversized query text handling.
- **Rust Checks**: `cargo check --manifest-path crates/contextra-py/Cargo.toml --all-features` (0 errors, 0 warnings).
- **Clippy Analysis**: `cargo clippy --manifest-path crates/contextra-py/Cargo.toml -- -D warnings` (0 findings).
- **Formatting**: `cargo fmt --check --manifest-path crates/contextra-py/Cargo.toml` (0 diffs).
- **Python Integration Suite**: `maturin develop --release` + `pytest` executed 52 test cases with 100% pass rate in virtualenv context.
...
- **Inventory Reality Check**: Confirmed `crates/contextra-py/src/lib.rs` (1639 lines) matches actual repo inventory with 0 drift (Stand: 2026-09-10).
- **Rust Unit & Sanity Checks**: `cargo check --manifest-path crates/contextra-py/Cargo.toml --all-features` (0 errors, 0 warnings).
- **Clippy Analysis**: `cargo clippy --manifest-path crates/contextra-py/Cargo.toml -- -D warnings` (0 findings).
- **Formatting**: `cargo fmt --check --manifest-path crates/contextra-py/Cargo.toml` (0 diffs).
...
## Audit Verification & Test Delta (Session `3a641825`, TS: 2026-09-09T19:13:19Z)

- **Inventory Reality Check**: Confirmed `crates/contextra-py/src/lib.rs` (1618 lines) matches actual repo inventory with 0 drift.
- **Rust Unit & Sanity Checks**: `cargo check --manifest-path crates/contextra-py/Cargo.toml --all-features` (0 errors, 0 warnings).
- **Clippy Analysis**: `cargo clippy --manifest-path crates/contextra-py/Cargo.toml -- -D warnings` (0 findings).
- **Formatting**: `cargo fmt --check --manifest-path crates/contextra-py/Cargo.toml` (0 diffs).
- **Python FFI / Integration Suite**: `maturin develop --release` + `pytest` executed 51 test cases with 100% pass rate in virtualenv context.
- **Tier 1 Concurrency Stress Test**: 10 consecutive runs of the entire `pytest` suite (510 total test executions) completed with 0 failures, 0 deadlocks, and 0 panics.
- **Sub-Interpreter Isolation & GIL Release**: Confirmed CPython sub-interpreter imports are cleanly rejected with explicit `ImportError` and GIL release protocol (`py.allow_threads`) functions under multi-threaded Python queries.
- **Preflight Check**: `cargo run -p xtask -- jules-preflight --fast` (100% PASSED).

---

## Audit Verification & Test Delta (Session `64f05109`, TS: 2026-09-09T15:57:24Z)

- **Inventory Reality Check**: Confirmed `crates/contextra-py/src/lib.rs` (1598 lines) matches actual repo inventory with 0 drift.
- **Rust Unit & Sanity Checks**: `cargo check --manifest-path crates/contextra-py/Cargo.toml --all-features` (0 errors, 0 warnings).
- **Clippy Analysis**: `cargo clippy --manifest-path crates/contextra-py/Cargo.toml -- -D warnings` (0 findings).
- **Rust Unit Test Suite**: `cargo test --manifest-path crates/contextra-py/Cargo.toml --all-features` (100% passed).
...
|---|---|---|---|---|---|
| `AGT-PY-d5d2be30` | SECURITY | MAJOR | `crates/contextra-py/src/lib.rs` | 293 | `panic = "abort"` in workspace `Cargo.toml` release profile disables `catch_unwind` panic containment in release builds |
...
- **Rust Unit & Sanity Checks**: `cargo check -p contextra-py --all-features` (0 errors, 0 warnings).
- **Clippy Analysis**: `cargo clippy -p contextra-py --no-deps -- -D warnings` (0 findings).
- **Rust Integration Test Suite**: `cargo test -p contextra-py --all-features` (100% passed).
- **Tier 1 Concurrency Verification**: 5 consecutive runs of `cargo test -p contextra-py --all-features -- --test-threads=8` executed with 0 failures and 0 panics.
...
| ID | Kategorie | Severity | Datei | Zeile | Beschreibung |
|---|---|---|---|---|---|
| `AGT-PY-ff475c8e` | BUG | MAJOR | `crates/contextra-py/src/lib.rs` | 1435 | `_trigger_panic_for_test` returns `PyRuntimeError` directly instead of panicking inside `run_blocking_ffi` |
...
| Disk-Full ENOSPC | OK | Storage error maps to ContextraIOError, no panic | — |
| OOM / Backpressure | OK | MAX_BATCH_SIZE (10,000) & thread pool bounds enforced | — |
| SIGBUS mmap-truncate | N/A | No direct mmap usage in contextra-py | — |
...
1. **Inventory Verification & Gate Alignment**:
   - Performed inventory reality check (`crates/contextra-py/src/lib.rs`).
...
2. **ANCHOR Review Status Update**:
   - Verified integration test anchors `ANCHOR[TEST:PY-001]` and `ANCHOR[TEST:PY-002]` in `crates/contextra-py/tests/test_bindings.py`, attaching `REVIEW-PASS[2/2]` for session `64f05109`.

### Changes Implemented in Session `9cd9a63a`

1. **`crates/contextra-py/src/lib.rs`**:
   - Enhanced `validate_id` to enforce `id.len() <= MAX_ID_LENGTH` (1024 bytes), returning `ContextraValueError` on violation.
   - Enhanced `validate_collection_name` to enforce `name.len() <= 64` bytes, returning `ContextraValueError` on violation.
2. **`crates/contextra-py/tests/test_errors.py`**:
...
1. **`crates/contextra-py/src/lib.rs`**:
   - Standardized `validate_collection_name`, `validate_db_path`, and `validate_query_text` to return `ContextraValueError` instead of standard `PyValueError`.
2. **`crates/contextra-py/tests/test_errors.py`**:
   - Added `test_empty_collection_name_and_query_validation` testing `ContextraValueError` raising behavior on empty collection names and empty hybrid search queries.
3. **`crates/contextra-py/tests/test_bindings.py` & `crates/contextra-py/AGENTS.md`**:
...
2. **`crates/contextra-py/src/lib.rs`**:
   ...
   - Added `test_contextra_err_conflict_and_sandbox` testing PyErr mapping for `ContextraError::Conflict` -> `PyRuntimeError` and `ContextraError::Sandbox` -> `PyPermissionError`.
3. **`crates/contextra-py/tests/test_errors.py` & `crates/contextra-py/tests/test_bindings.py`**:
...
1. **`crates/contextra-py/src/lib.rs`**:
   ...
   - Refactored `relate` implementation in `contextra_crud_methods!` macro to delegate label validation to `validate_label`.
