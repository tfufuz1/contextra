# AUDIT: Fusion-Grenzflächen (`contextra-graph` PPR/CSR vs. `contextra-db` Fusion)

**Datum:** 2026-09-15
**Crate:** `contextra-graph` (Layer 2) & `contextra-db` (Layer 3)
**Status:** COMPLETE (Read-Only Audit)
**Auditierte Dateien:**
- `crates/contextra-graph/src/ppr.rs`
...
Das Modul `contextra-graph` stellt über die Compressed Sparse Row (CSR) Graph-Implementierung (`CsrGraph`) Signal 3 der 4-Signal-Fusion im Contextra Cognitive OS bereit.
In diesem Audit wurden die Schnittstellen zwischen `contextra-graph` (`ppr.rs`, `csr.rs`) und der Fusion/Retrieval-Schicht in `contextra-db` (`fusion.rs`, `collection/search.rs`) auf Blockierverhalten, Fehlerbehandlung, ID-Herkunft, Tombstone-Sichtbarkeit und Timeouts untersucht.

---

## 2. ABGLEICHS-AUFTRAG: FRAGEN & BELEGE

### Frage 1: Öffentliche Funktionssignaturen für `contextra-db/src/fusion.rs`
**Frage:** Welche öffentlichen Funktionssignaturen aus `ppr.rs`/`csr.rs` werden von `contextra-db/src/fusion.rs` konsumiert? Vollständige Signaturen.

- **Befund:** **KEINE (0 Funktionssignaturen)**.
- **Analyse:** `contextra-db/src/fusion.rs` ist ein reine Reciprocal Rank Fusion (RRF) Berechnungseinheit. Sie konsumiert weder `ppr.rs` noch `csr.rs` direkt, sondern arbeitet ausschließlich auf universellen `SearchResult`-Strukturen (`Vec<SearchResult>` bzw. `Vec<(String, Vec<SearchResult>, f32)>`).
- **Grenzflächen-Fluss:** Die Interaktion mit `CsrGraph` erfolgt in `contextra-db/src/collection/search.rs`. Dort ruft `Collection` Traversal-Methoden des `GraphIndex`-Traits (wie `multi_traverse_at`) auf, wandelt die Ergebnisse in `SearchResult`-Kandidaten um und übergibt diese an `fusion::weighted_reciprocal_rank_fusion_with_options`.
- **Exakte Fundstellen:**
  - `crates/contextra-db/src/fusion.rs:453–459`:
  ...
  - `crates/contextra-db/src/collection/search.rs:750, 1101`: Aufruf von `self.graph_index.multi_traverse_at(...)`.
- **Status:** **BELEGT**
...
- **Exakte Fundstellen:**
  - `crates/contextra-graph/src/csr.rs:310`: `write_state: Arc<Mutex<GraphInner>>`
  - `crates/contextra-graph/src/csr.rs:374–376`: `pub(crate) fn inner_read(&self) -> arc_swap::Guard<Arc<GraphInner>> { self.inner.load() }`
  - `crates/contextra-graph/src/csr.rs:736`: `let mut inner_writer = write_state.lock();`
  - `crates/contextra-graph/src/csr.rs:885–897`: `personalized_page_rank_with_context_async`
  - `crates/contextra-graph/src/ppr.rs:108`: `pub(crate) fn compute_ppr_with_context(...)`
- **Status:** **BELEGT**
...
- **Exakte Fundstellen:**
  - `crates/contextra-graph/src/ppr.rs:116–118`
  - `crates/contextra-graph/src/ppr.rs:137–139`
  - `crates/contextra-graph/src/csr.rs:896`
  - `crates/contextra-graph/src/csr.rs:988`
- **Status:** **BELEGT**
...
- **Analyse:**
  ...
  2. In `contextra-db/src/collection/search.rs` teilen sich Dokumente und Graphknoten dieselben Schlüssel: `EntityId::from_key(doc.id.as_str())`.
...
- **Exakte Fundstellen:**
  - `crates/contextra-graph/src/csr.rs:200–202`: `id_map` und `reverse_map`
  - `crates/contextra-graph/src/csr.rs:252–265`: `get_or_create_index(id: EntityId) -> InternalIndex`
  - `crates/contextra-graph/src/csr.rs:468–481`: `add_edge(...)` Akzeptanz von `from`, `to` und `source_doc_id`
  - `crates/contextra-db/src/collection/search.rs:739, 1083`: Mapping via `EntityId::from_key(r.id.as_str())`
- **Status:** **BELEGT**
...
- **Exakte Fundstellen:**
  - `crates/contextra-graph/src/ppr.rs:125, 150, 165, 178, 191, 207, 241, 258`
  - `crates/contextra-graph/src/csr.rs:350–360` (Kanten-Tombstone-Filterung in `compact()`)
  - `crates/contextra-graph/src/csr.rs:728–730` (`compact_async` Budget-Deferral)
  - `crates/contextra-graph/src/csr.rs:886–895` (`personalized_page_rank_with_context_async` Reihenfolge)
- **Status:** **BELEGT**
...
- **Analyse:**
  ...
  3. In `contextra-db/src/collection/search.rs` wird PPR unter MVCC-Snapshot-Isolation derzeit explizit abgelehnt (`ContextraError::SnapshotUnsupportedForSignal`, `search.rs:756–760, 1106–1110`).
  4. `fusion.rs` hat keinerlei Kontrolle oder Parameter bezüglich Wall-Clock-Timeouts für Graph-Traversierungen.
- **Exakte Fundstellen:**
  - `crates/contextra-graph/src/csr.rs:885–897`
  - `crates/contextra-graph/src/ppr.rs:154`: `let max_iters = config.max_iterations.min(1000);`
  - `crates/contextra-db/src/collection/search.rs:756–760, 1106–1110`
- **Status:** **BELEGT**
...
### 3.2 Koppelung an MVCC Snapshot Isolation in `contextra-db`
...
- `PersonalizedPageRank` operiert auf dem un-versionierten Live-Graph-Zustand. Daher wird PPR in `search.rs` bei aktiver Snapshot-Isolation mit `ContextraError::SnapshotUnsupportedForSignal` abgewiesen.

---

## 4. VERIFIKATIONS-VERDIKT

Alle 6 Kernfragen zur Fusion-Grenzfläche zwischen `contextra-graph` und `contextra-db` wurden anhand des aktuellen Quellcodes vollständig und präzise beantwortet und belegt. Es wurden keine Modifikationen am Produktionscode vorgenommen.
